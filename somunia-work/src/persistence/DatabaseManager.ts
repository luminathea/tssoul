...
export class DatabaseManager {
  ...
  constructor(config: Partial<DatabaseConfig> = {}) {
    ...
    this.db = new Database(this.config.dbPath, {
      verbose: this.config.verbose ? (msg?: unknown) => console.log(`[DB] ${msg}`) : undefined,
    });
  ...
  private getSchemas(): SchemaMigration[] {
    return [
      // ============================================================
      // v1: 基本テーブル群
      // ============================================================
  ...
  /** ノードを追加または更新 */
  upsertNode(node: {
    id: string; type: string; label: string; activation?: number;
    lastActivated?: number; createdAt?: number; useCount?: number;
    emotionalColor?: string | null; meta?: Record<string, any>;
  }): void {
    this.db.prepare(`
    ...
    `).run(
      ...
      node.activation ?? 0, node.lastActivated ?? 0, node.createdAt ?? 0,
      node.useCount ?? 0, node.emotionalColor ?? null,
  ...
  upsertEdge(edge: {
    ...
    relation: string; lastUsed?: number; useCount?: number;
  }): void {
    this.db.prepare(`
    ...
    `).run(
      ...
      edge.relation, edge.lastUsed ?? 0, edge.useCount ?? 0
    );
  }

  /** ノードIDで隣接ノードを取得 */
  getAdjacentNodes(nodeId: string, minWeight: number = 0.01): Array<{
    node_id: string; label: string; type: string; weight: number;
    relation: string; activation: number; emotional_color: string | null;
  }> {
    return this.all(`
  ...
  /** 活性度上位のノードを取得 */
  getActiveNodes(limit: number = 20): any[] {
    return this.all(
      'SELECT * FROM assoc_nodes WHERE activation > 0.01 ORDER BY activation DESC LIMIT ?',
      limit
    );
  }
  ...
  /** 全ノード数 */
  getNodeCount(): number {
    const row = this.get<{ count: number }>('SELECT COUNT(*) as count FROM assoc_nodes');
    return row?.count ?? 0;
  }

  /** 全エッジ数 */
  getEdgeCount(): number {
    const row = this.get<{ count: number }>('SELECT COUNT(*) as count FROM assoc_edges');
    return row?.count ?? 0;
  }
  ...
  saveEpisodicMemory(memory: {
    id: string; content: string; summary?: string; timestamp: number;
    duration?: number; emotionalTags?: string[]; emotionalIntensity?: number;
    importance?: number; relatedMemories?: string[]; relatedConcepts?: string[];
    recallCount?: number; lastRecalled?: number | null; retentionStrength?: number;
  }): void {
    this.db.prepare(`
    ...
    `).run(
      ...
      memory.timestamp, memory.duration ?? 0,
      JSON.stringify(memory.emotionalTags ?? []),
      memory.emotionalIntensity ?? 0,
      memory.importance ?? 0.5,
      JSON.stringify(memory.relatedMemories ?? []),
      JSON.stringify(memory.relatedConcepts ?? []),
      memory.recallCount ?? 0, memory.lastRecalled ?? null,
      memory.retentionStrength ?? 1.0
    );
  ...
  /** 重要度順にエピソード記憶を取得 */
  getEpisodicMemories(limit: number = 50, minImportance: number = 0): any[] {
    return this.all(
      'SELECT * FROM episodic_memories WHERE importance >= ? ORDER BY importance DESC, timestamp DESC LIMIT ?',
      minImportance, limit
    );
  }

  /** テキスト検索でエピソード記憶を検索 */
  searchEpisodicMemories(query: string, limit: number = 10): any[] {
    return this.all(`
  ...
  saveSemanticMemory(memory: {
    id: string; concept: string; definition?: string; source?: string;
    learnedAt?: number; comprehension?: number; interestLevel?: number;
    useCount?: number; lastUsed?: number | null;
    relations?: Array<{ concept: string; relationType: string; strength: number }>;
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      memory.id, memory.concept, memory.definition ?? '',
      memory.source ?? 'initial', memory.learnedAt ?? 0,
      memory.comprehension ?? 0.5, memory.interestLevel ?? 0.5,
      memory.useCount ?? 0, memory.lastUsed ?? null
    );
    ...
    if (memory.relations && memory.relations.length > 0) {
      ...
      const insertRel = this.db.prepare(
        'INSERT INTO concept_relations (memory_id, related_concept, relation_type, strength) VALUES (?, ?, ?, ?)'
      );
  ...
  /** 概念検索 */
  searchConcepts(query: string, limit: number = 10): any[] {
    return this.all(`
  ...
  saveVisitor(visitor: {
    ...
    callName?: string | null; firstVisitAt: number; lastVisitAt: number;
    visitCount?: number; communicationStyle?: any; isCurrent?: boolean;
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      visitor.id, visitor.name ?? null, visitor.nickname ?? null,
      visitor.callName ?? null, visitor.firstVisitAt, visitor.lastVisitAt,
      visitor.visitCount ?? 0, JSON.stringify(visitor.communicationStyle ?? {}),
      visitor.isCurrent ? 1 : 0
    );
  }

  /** 訪問者のfactを追加 */
  addVisitorFact(visitorId: string, fact: {
    category: string; content: string; confidence?: number;
    learnedAt: number; source?: string;
  }): void {
    this.db.prepare(`
      INSERT INTO visitor_facts (visitor_id, category, content, confidence, learned_at, source)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(
      visitorId, fact.category, fact.content,
      fact.confidence ?? 0.5, fact.learnedAt, fact.source ?? 'conversation'
    );
  }
  ...
  saveConversation(conv: {
    id: string; visitorId?: string | null; startedAt: number;
    endedAt?: number | null; mood?: string; topics?: string[];
    messageCount?: number; avgSatisfaction?: number; summary?: string | null;
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      conv.id, conv.visitorId ?? null, conv.startedAt,
      conv.endedAt ?? null, conv.mood ?? 'casual',
      JSON.stringify(conv.topics ?? []),
      conv.messageCount ?? 0, conv.avgSatisfaction ?? 0,
      conv.summary ?? null
    );
  }

  saveMessage(msg: {
    id: string; conversationId: string; speaker: string; content: string;
    timestamp: number; emotionalContext?: string | null;
    thoughtBefore?: string | null; emotionDuring?: string | null;
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      msg.id, msg.conversationId, msg.speaker, msg.content,
      msg.timestamp, msg.emotionalContext ?? null,
      msg.thoughtBefore ?? null, msg.emotionDuring ?? null,
      msg.satisfactionAfter ?? null
    );
  ...
  saveResponsePattern(pattern: {
    id: string; template: string; successCount?: number; useCount?: number;
    avgSatisfaction?: number; lastUsed?: number; origin?: string;
    emotionTags?: string[]; situation?: {
      intents?: string[]; emotions?: string[]; depths?: string[];
      timeOfDay?: string[]; relationshipPhases?: string[]; keywords?: string[];
    };
  }): void {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      pattern.id, pattern.template,
      pattern.successCount ?? 0, pattern.useCount ?? 0,
      pattern.avgSatisfaction ?? 0, pattern.lastUsed ?? 0,
      pattern.origin ?? 'extracted',
      JSON.stringify(pattern.emotionTags ?? []),
      JSON.stringify(sit.intents ?? []),
      JSON.stringify(sit.emotions ?? []),
      JSON.stringify(sit.depths ?? []),
      JSON.stringify(sit.timeOfDay ?? []),
      JSON.stringify(sit.relationshipPhases ?? []),
      JSON.stringify(sit.keywords ?? [])
    );
  }
  ...
  saveDiaryEntry(entry: {
    id: string; date: string; content: string; mood?: string;
    moodIntensity?: number; weather?: string | null; dayNumber?: number;
    createdAt: number; events?: Array<{
      time: string; description: string; significance?: number; emotion?: string | null;
    }>;
  }): void {
    this.db.prepare(`
    ...
    `).run(
      ...
      entry.mood ?? 'peace', entry.moodIntensity ?? 0.5,
      entry.weather ?? null, entry.dayNumber ?? 1, entry.createdAt
    );

    if (entry.events && entry.events.length > 0) {
      this.db.prepare('DELETE FROM diary_events WHERE entry_id = ?').run(entry.id);
      const insertEvent = this.db.prepare(
        'INSERT INTO diary_events (entry_id, time, description, significance, emotion) VALUES (?, ?, ?, ?, ?)'
      );
  ...
  saveCreativeWork(work: {
    ...
    createdAt: number; emotionAtCreation?: string | null;
    inspiration?: string | null; qualitySelfRating?: number | null;
    tags?: string[];
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      work.id, work.type, work.title ?? '', work.content,
      work.createdAt, work.emotionAtCreation ?? null,
      work.inspiration ?? null, work.qualitySelfRating ?? null,
  ...
  recordEmotionSnapshot(snapshot: {
    tick: number; dominantEmotion: string; valence: number;
    arousal: number; emotionVector: Record<string, number>;
    trigger?: string | null;
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(
      snapshot.tick, snapshot.dominantEmotion,
      snapshot.valence, snapshot.arousal,
      JSON.stringify(snapshot.emotionVector),
      snapshot.trigger ?? null
    );
  }

  /** 最近の感情推移 */
  getEmotionHistory(limit: number = 100): any[] {
    return this.all(
      'SELECT * FROM emotion_history ORDER BY tick DESC LIMIT ?',
      limit
    );
  }
  ...
  loadModuleState(moduleName: string): any | null {
    const row = this.get<{ state: string }>(
      'SELECT state FROM module_states WHERE module_name = ?',
      moduleName
    );
    if (!row) return null;
    try {
  ...
  /** 全モジュール状態のリストを取得 */
  listModuleStates(): Array<{ module_name: string; size_bytes: number; updated_at: string }> {
  ...
  getSystemState(key: string): any | null {
    const row = this.get<{ value: string }>('SELECT value FROM system_state WHERE key = ?', key);
    if (!row) return null;
    try {
  ...
  saveProceduralMemory(memory: {
    id: string; skill: string; steps?: string[]; proficiency?: number;
    practiceCount?: number; lastExecuted?: number | null; automatization?: number;
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      memory.id, memory.skill,
      JSON.stringify(memory.steps ?? []),
      memory.proficiency ?? 0, memory.practiceCount ?? 0,
      memory.lastExecuted ?? null, memory.automatization ?? 0
    );
  }
  ...
  saveHabit(habit: {
    ...
    triggerCondition?: any; actions?: string[]; frequency?: number;
    strength?: number; lastExecuted?: number | null; executionCount?: number;
    satisfactionAvg?: number; active?: boolean;
  }): void {
    this.db.prepare(`
    ...
    `).run(
      ...
      habit.triggerType ?? 'time', JSON.stringify(habit.triggerCondition ?? {}),
      JSON.stringify(habit.actions ?? []), habit.frequency ?? 0,
      habit.strength ?? 0, habit.lastExecuted ?? null,
      habit.executionCount ?? 0, habit.satisfactionAvg ?? 0.5,
      habit.active !== false ? 1 : 0
    );
  }
  ...
  saveDream(dream: {
    id: string; phase: string; content?: string; themes?: string[];
    emotionalTone?: string | null; vividness?: number;
    startedAt: number; endedAt?: number | null; memoriesConsolidated?: number;
    fragments?: Array<{
      content: string; type?: string; emotion?: string | null; sourceMemoryId?: string | null;
    }>;
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      dream.id, dream.phase, dream.content ?? '',
      JSON.stringify(dream.themes ?? []),
      dream.emotionalTone ?? null, dream.vividness ?? 0.5,
      dream.startedAt, dream.endedAt ?? null,
      dream.memoriesConsolidated ?? 0
    );
    ...
    if (dream.fragments && dream.fragments.length > 0) {
      ...
      const insertFrag = this.db.prepare(
        'INSERT INTO dream_fragments (dream_id, content, type, emotion, source_memory_id) VALUES (?, ?, ?, ?, ?)'
      );
  ...
  saveLearningSession(session: {
    id: string; topic: string; source?: string; contentSummary?: string;
    learnedAt: number; comprehension?: number; interestLevel?: number;
    conceptsLearned?: string[]; relatedSearches?: string[];
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      session.id, session.topic, session.source ?? 'wikipedia',
      session.contentSummary ?? '', session.learnedAt,
      session.comprehension ?? 0.5, session.interestLevel ?? 0.5,
      JSON.stringify(session.conceptsLearned ?? []),
      JSON.stringify(session.relatedSearches ?? [])
    );
  }
  ...
  saveSelfModification(mod: {
    id: string; type: string; beforeState?: string; afterState?: string;
    reason?: string; trigger?: string; impact?: number;
    timestamp: number; reverted?: boolean;
  }): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      mod.id, mod.type, mod.beforeState ?? '', mod.afterState ?? '',
      mod.reason ?? '', mod.trigger ?? '', mod.impact ?? 0,
      mod.timestamp, mod.reverted ? 1 : 0
    );
  }
  ...
  getStats(): {
    nodeCount: number; edgeCount: number; episodicCount: number;
    semanticCount: number; proceduralCount: number; visitorCount: number;
    conversationCount: number; messageCount: number; patternCount: number;
    diaryCount: number; creativeCount: number; dreamCount: number;
    dbSizeBytes: number;
  } {
    const count = (table: string): number => {
      const row = this.get<{ c: number }>(`SELECT COUNT(*) as c FROM ${table}`);
      return row?.c ?? 0;
    };
