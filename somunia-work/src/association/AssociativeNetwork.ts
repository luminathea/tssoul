...
const DEFAULT_CONFIG: AssociativeNetworkConfig = {
  maxNodes: 2000,
  maxEdgesPerNode: 30,
...
const INITIAL_NODES: Array<Omit<AssocNode, 'id' | 'activation' | 'lastActivated' | 'createdAt' | 'useCount'>> = [
  // 自分自身
...
const INITIAL_EDGES: Array<{ from: string; to: string; relation: AssocRelation; weight: number }> = [
...
export class AssociativeNetwork {
  private config: AssociativeNetworkConfig;
  private nodes: Map<ID, AssocNode> = new Map();
  private edges: AssocEdge[] = [];
  private labelToId: Map<string, ID> = new Map();
  private nodeEdgeIndex: Map<ID, AssocEdge[]> = new Map();
  ...
  private initializeNetwork(): void {
    ...
    for (const edgeDef of INITIAL_EDGES) {
      ...
      if (fromId && toId) {
        this.addEdge(fromId, toId, edgeDef.relation, edgeDef.weight as Normalized);
      }
    }
  }
  ...
  addNode(
    type: AssocNodeType,
    label: string,
    emotionalColor: EmotionType | null = null,
  ...
  addEdge(
    ...
    if (existing) {
      existing.weight = Math.min(1, existing.weight + weight * 0.3) as Normalized;
      existing.useCount++;
      return existing;
    }
    ...
    const edge: AssocEdge = {
      fromId,
      toId,
      weight: Math.min(1, weight) as Normalized,
      relation,
      lastUsed: 0,
      useCount: 1,
    };
  ...
  private findEdge(fromId: ID, toId: ID): AssocEdge | null {
   ...
   */
  activate(nodeId: ID, intensity: Normalized = 0.8, tick: Tick = 0): AssocNode[] {
    const node = this.nodes.get(nodeId);
    if (!node) return [];

    const activated: AssocNode[] = [];
    const visited = new Set<ID>();
    const queue: Array<{ id: ID; depth: number; energy: number }> = [
    ...
    while (queue.length > 0) {
      ...
      if (current.depth > this.config.spreadingActivationDepth) continue;
      if (current.energy < 0.05) continue;
      ...
      // ノードを活性化
      n.activation = Math.min(1, n.activation + current.energy) as Normalized;
  ...
  /** ラベル名でアクティベート */
  activateByLabel(label: string, intensity: Normalized = 0.8, tick: Tick = 0): AssocNode[] {
    const id = this.labelToId.get(label);
    if (!id) return [];
   ...
   */
  activateMultiple(labels: string[], intensity: Normalized = 0.6, tick: Tick = 0): AssocNode[] {
    const allActivated = new Map<ID, AssocNode>();
    ...
    for (const label of labels) {
      ...
      for (const node of activated) {
        if (allActivated.has(node.id)) {
          ...
          existing.activation = Math.min(1, existing.activation + node.activation * 0.3) as Normalized;
        } else {
  ...
  private hebbianLearning(activeNodes: AssocNode[], tick: Tick): void {
    for (let i = 0; i < activeNodes.length; i++) {
      for (let j = i + 1; j < Math.min(i + 5, activeNodes.length); j++) {
        ...
        if (existingEdge) {
          // 既存エッジを強化
          existingEdge.weight = Math.min(1,
            existingEdge.weight + this.config.hebbianLearningRate * a.activation * b.activation
          ) as Normalized;
          existingEdge.lastUsed = tick;
          existingEdge.useCount++;
        } else if (a.activation > 0.5 && b.activation > 0.5) {
          // 新しい連想の発見
          this.addEdge(a.id, b.id, 'related_to', 0.1 as Normalized);
        }
      }
    }
  }
  ...
  tick(currentTick: Tick): void {
    ...
    for (const node of this.nodes.values()) {
      if (node.activation > 0) {
        node.activation = Math.max(0,
          node.activation - this.config.activationDecayRate
        ) as Normalized;
      }
    }
      ...
      for (const edge of this.edges) {
        if (edge.weight > this.config.minEdgeWeight) {
          edge.weight = Math.max(
            ...
            edge.weight - this.config.edgeDecayRate
          ) as Normalized;
        }
      }
      // 弱いエッジを除去
  ...
  getAssociations(label: string, limit: number = 10): Array<{ node: AssocNode; weight: number }> {
    const id = this.labelToId.get(label);
    if (!id) return [];

    const edges = this.nodeEdgeIndex.get(id) || [];
    const results: Array<{ node: AssocNode; weight: number }> = [];
  ...
  /** 現在活性化しているノードを取得 */
  getActiveNodes(minActivation: Normalized = 0.1): AssocNode[] {
  ...
  /** 感情に関連するノードを取得 */
  getEmotionalAssociations(emotion: EmotionType, limit: number = 5): AssocNode[] {
    return Array.from(this.nodes.values())
  ...
  /** ネットワーク統計 */
  getStats(): { nodeCount: number; edgeCount: number; avgDegree: number; activeCount: number } {
  ...
  recordExperience(
    ...
    for (const concept of relatedConcepts) {
      ...
      this.addEdge(expNode.id, conceptNode.id, 'related_to', 0.5 as Normalized);
    }
    ...
    if (emotionNode) {
      this.addEdge(expNode.id, emotionNode.id, 'feels_like', 0.6 as Normalized);
    }

    // 活性化
    this.activate(expNode.id, 0.5 as Normalized, tick);
  ...
  recordCreation(title: string, type: string, emotion: EmotionType, tick: Tick): AssocNode {
    ...
    if (selfNode) {
      this.addEdge(selfNode.id, node.id, 'created_during', 0.7 as Normalized);
    }
    return node;
  }
  ...
  recordDream(description: string, emotion: EmotionType, tick: Tick): AssocNode {
    ...
    if (dreamConcept) {
      this.addEdge(node.id, dreamConcept.id, 'part_of', 0.5 as Normalized);
    }
    return node;
  }
  ...
  private pruneWeakNodes(): void {
    ...
    for (const node of toRemove) {
      // 初期ノードは削除しない
      if (node.meta.isSelf || node.meta.isHome) continue;
  ...
  private pruneWeakEdges(nodeId: ID): void {
