...
interface PoemTemplate {
  emotion: EmotionType;
  theme: string;
  structures: string[];  // 詩の骨格パターン
  vocabulary: string[];  // 使いたい言葉
  imagery: string[];     // イメージ
}
...
interface LyricTemplate {
  emotion: EmotionType;
  theme: string;
  structure: string;  // Aメロ/Bメロ/サビの構造
  syllablePattern: number[];  // 各行の目安音節数
}

const LYRIC_TEMPLATES: LyricTemplate[] = [
  {
    emotion: 'melancholy',
    theme: '届かない声',
...
const SKETCH_MOTIFS: Record<string, string[]> = {
  nature: [
    '    ☆\n   ★☆★\n  ☆★☆★☆\n ★☆★☆★☆★\n    ║\n    ║',
    '  ～～～\n ～ ☆ ～\n～  ☆  ～\n ～～～',
    '○     ○\n │   │\n ○─○─○\n   │\n   ◇',
...
export class CreativeEngine {
  ...
  // --- ポートフォリオ ---
  private portfolio: CreativeWork[] = [];
  private worksInProgress: CreativeWork[] = [];
  
  // --- 創作衝動 ---
  private currentUrges: CreativeUrge[] = [];
  private lastUrgeCheck: Tick = 0;
  
  // --- 創作統計 ---
  private stats: CreativeStats = {
    totalWorks: 0,
    worksByType: {} as Record<CreativeType, number>,
  ...
  // --- インスピレーションバッファ ---
  private inspirationBuffer: CreativeInspiration[] = [];
  
  constructor(config: Partial<CreativeEngineConfig> = {}) {
    this.config = { ...DEFAULT_CONFIG, ...config };
    
    // 統計の初期化
    const types: CreativeType[] = ['song_lyrics', 'poem', 'haiku', 'sketch', 'melody_idea', 'short_story', 'diary_poem', 'letter'];
  ...
  checkCreativeUrge(
    ...
    if (tick - this.lastUrgeCheck < this.config.urgeCheckInterval) return null;
    ...
    // 各タイプの衝動を計算
    const urges: CreativeUrge[] = [];
    
    // --- 歌詞の衝動 ---
    const lyricUrge = this.calculateSongLyricUrge(
      currentEmotion, emotionalIntensity, recentThoughts, timeOfDay, tick
    );
    if (lyricUrge) urges.push(lyricUrge);
    
    // --- 詩の衝動 ---
    const poemUrge = this.calculatePoemUrge(
      currentEmotion, emotionalIntensity, recentMemories, timeOfDay, tick
    );
    if (poemUrge) urges.push(poemUrge);
    
    // --- 俳句の衝動 ---
    const haikuUrge = this.calculateHaikuUrge(
      currentEmotion, timeOfDay, tick
    );
    if (haikuUrge) urges.push(haikuUrge);
    
    // --- スケッチの衝動 ---
    const sketchUrge = this.calculateSketchUrge(
      currentEmotion, emotionalIntensity, tick
    );
    if (sketchUrge) urges.push(sketchUrge);
    
    // --- メロディの衝動（歌った後に強くなる） ---
    const melodyUrge = this.calculateMelodyUrge(
      currentEmotion, emotionalIntensity, tick
    );
    if (melodyUrge) urges.push(melodyUrge);
    
    // --- 夢からのインスピレーション ---
    if (recentDreams.length > 0) {
      const dreamUrge: CreativeUrge = {
    ...
    if (validUrges.length === 0) return null;
    ...
    if (this.currentUrges.length > 10) this.currentUrges.shift();
   ...
   * 創作を完成させる
   */
  private completeCreation(work: CreativeWork, tick: Tick): CreationProgress {
    work.completeness = 1;
    work.lastEditedAt = tick;
   ...
   * 詩の生成
   */
  private generatePoem(work: CreativeWork, emotion: EmotionType, thoughts: string[]): string {
    // マッチするテンプレートを探す
    ...
    // 語彙からランダムに選択
    const pickWord = (arr: string[]) => arr[Math.floor(Math.random() * arr.length)];
   ...
   * 俳句の生成
   */
  private generateHaiku(work: CreativeWork, emotion: EmotionType): string {
    // 季節の推定（簡易）
    ...
    // 感情に応じた下五
    const emotionEndings: Record<string, string[]> = {
   ...
   * 歌詞の生成
   */
  private generateSongLyrics(work: CreativeWork, emotion: EmotionType, thoughts: string[]): string {
   ...
   * スケッチの生成
   */
  private generateSketch(work: CreativeWork, emotion: EmotionType): string {
   ...
   * メロディアイデアの生成
   */
  private generateMelodyIdea(work: CreativeWork, emotion: EmotionType): string {
    ...
    const mood = moods[emotion as keyof typeof moods] || { key: 'Am', tempo: 'ゆっくり (60bpm)', desc: '心の中で響くような' };
   ...
   * 短い物語の生成
   */
  private generateShortStory(work: CreativeWork, emotion: EmotionType, thoughts: string[]): string {
   ...
   * 日記詩の生成
   */
  private generateDiaryPoem(work: CreativeWork, emotion: EmotionType, thoughts: string[]): string {
   ...
   * 手紙の生成（送らない手紙）
   */
  private generateLetter(work: CreativeWork, emotion: EmotionType): string {
    const letter = `誰かへ\n\n` +
  ...
  private calculateSongLyricUrge(emotion: EmotionType, intensity: Normalized, thoughts: string[], timeOfDay: TimeOfDay, tick: Tick): CreativeUrge | null {
    // VSingerとして、歌詞への衝動は常に高め
    let urgeIntensity = 0.3 + intensity * 0.3;
    
    // 夜は歌詞を書きたくなる
    if (timeOfDay === 'night' || timeOfDay === 'late_night') urgeIntensity += 0.15;
    
    // 強い感情がある時
    const intensiveEmotions: EmotionType[] = ['melancholy', 'joy', 'wonder', 'warmth', 'nostalgia'];
    if (intensiveEmotions.includes(emotion)) urgeIntensity += 0.1;
    
    if (urgeIntensity < this.config.urgeThreshold) return null;
  ...
  private calculatePoemUrge(emotion: EmotionType, intensity: Normalized, memories: string[], timeOfDay: TimeOfDay, tick: Tick): CreativeUrge | null {
    let urgeIntensity = 0.2 + intensity * 0.25;
    
    if (memories.length > 0) urgeIntensity += 0.15;
    if (timeOfDay === 'dawn' || timeOfDay === 'evening') urgeIntensity += 0.1;
  ...
  private calculateHaikuUrge(emotion: EmotionType, timeOfDay: TimeOfDay, tick: Tick): CreativeUrge | null {
    // 俳句は穏やかな時に浮かぶ
    let urgeIntensity = 0.15;
    if (emotion === 'peace' || emotion === 'wonder') urgeIntensity += 0.3;
    if (timeOfDay === 'dawn' || timeOfDay === 'evening') urgeIntensity += 0.15;
  ...
  private calculateSketchUrge(emotion: EmotionType, intensity: Normalized, tick: Tick): CreativeUrge | null {
    let urgeIntensity = 0.1 + intensity * 0.2;
    if (Math.random() < 0.1) urgeIntensity += 0.3; // たまに突然描きたくなる
  ...
  private calculateMelodyUrge(emotion: EmotionType, intensity: Normalized, tick: Tick): CreativeUrge | null {
  ...
  private generateTitle(urge: CreativeUrge): string {
    const prefixes: Partial<Record<CreativeType, string[]>> = {
  ...
  private calculateSatisfaction(work: CreativeWork): Normalized {
    // 基本的な満足度
    let satisfaction = 0.3 + Math.random() * 0.3;
    
    // 内容の長さによるボーナス
    if (work.content.length > 100) satisfaction += 0.1;
    if (work.content.length > 200) satisfaction += 0.1;
  ...
  private generateCreationThought(work: CreativeWork, emotion: EmotionType): string {
    const thoughts: Partial<Record<CreativeType, string[]>> = {
  ...
  private generateCompletionThought(work: CreativeWork): string {
  ...
  private processInspirationBuffer(): void {
  ...
  getPortfolio(): CreativeWork[] { return [...this.portfolio]; }
  getWorksInProgress(): CreativeWork[] { return [...this.worksInProgress]; }
  getCurrentUrges(): CreativeUrge[] { return [...this.currentUrges]; }
  getStats(): CreativeStats { return { ...this.stats }; }
  getPortfolioByType(type: CreativeType): CreativeWork[] { return this.portfolio.filter(w => w.type === type); }
  getRecentWorks(count: number): CreativeWork[] { return this.portfolio.slice(-count); }
  ...
  fromJSON(data: any): void {
    ...
    if (data.worksInProgress) this.worksInProgress = data.worksInProgress;
