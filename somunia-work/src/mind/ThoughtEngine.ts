...
const THOUGHT_TEMPLATES: ThoughtTemplate[] = [
  ...
  // === 感覚的思考 ===
  {
    ...
    emotionalColors: ['peace', 'comfort' as any],
    baseIntensity: 0.2,
  },
...
export class ThoughtEngine {
  private recentThoughts: ThoughtNode[] = [];
  private maxRecentThoughts: number = 50;
  private thoughtHistory: ThoughtNode[] = [];
  private maxHistory: number = 500;
  private customTemplates: ThoughtTemplate[] = [];
  private lastThoughtTick: Tick = 0;
  private minThoughtInterval: number = 3; // 最低3ティックの間隔
  ...
  generateThought(context: ThoughtContext, tick: Tick): ThoughtNode | null {
    ...
    let selected: ThoughtTemplate | null = null;
    ...
    if (!selected) selected = candidates[0].template;

    // テンプレートからランダムに選択
    const content = selected.templates[
      Math.floor(Math.random() * selected.templates.length)
    ];

    // 感情色を選択
    const emotionalColor = selected.emotionalColors[
      Math.floor(Math.random() * selected.emotionalColors.length)
    ];
  ...
  generateYuragiThought(yuragiType: string, tick: Tick): ThoughtNode {
    const yuragiThoughts: Record<string, string[]> = {
      sudden_interest: [
        '...そういえば、あれって何だっけ',
        '突然、全然関係ないことが気になる...',
        'なぜだろう、急にあのことが頭に浮かんだ',
   ...
   * 連想を見つける
   */
  private findAssociations(content: string): ID[] {
    const associations: ID[] = [];
    // 最近の思考と内容を比較
   ...
   * 最近の思考を取得
   */
  getRecentThoughts(count: number = 10): ThoughtNode[] {
  ...
  fromJSON(data: any): void {
    if (data.recentThoughts) this.recentThoughts = data.recentThoughts;
    if (data.thoughtHistory) this.thoughtHistory = data.thoughtHistory;
    if (data.lastThoughtTick) this.lastThoughtTick = data.lastThoughtTick;
  }
}
