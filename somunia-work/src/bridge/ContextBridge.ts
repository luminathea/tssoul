...
const TIME_DESCRIPTIONS: Record<TimeOfDay, { description: string; ambiance: string[] }> = {
...
export class ContextBridge {
  ...
  constructor(
    selfModel: SelfModel,
    visitorMemory: VisitorMemorySystem,
  ...
  buildResponseContext(
    provider: SelfModelStateProvider,
    message: string,
    turnAnalysis: {
    ...
    // 3. 訪問者コンテキスト
    const visitor = this.visitorMemory.generateVisitorContext(
      turnAnalysis.topics, message, relationshipPhase
    );

    // 4. 名前検出処理
    const nameResult = this.visitorMemory.processMessage(
      message, turnAnalysis.emotions, turnAnalysis.topics,
      conversationState.depth, tick
    );

    // 5. 連想ネットワークの活性化
    const associationLabels = this.activateAssociations(
      turnAnalysis.topics, turnAnalysis.emotions, tick
    );
    ...
    // 7. 応答の指針
    const guideline = this.buildGuideline(
      turnAnalysis, conversationState.depth, relationshipPhase,
      selfSnapshot, visitor, nameResult.hasNameIntroduction
    );
  ...
  private buildMomentContext(provider: SelfModelStateProvider, tick: Tick): MomentContext {
    ...
    // 身体状態
    let physicalState = '';
    if (body.energy < 0.3) physicalState = '疲れてる';
    else if (body.sleepiness > 0.6) physicalState = '眠い';
    else physicalState = '普通';
  ...
  private activateAssociations(
    topics: string[], emotions: EmotionType[], tick: Tick
  ): string[] {
    const allLabels = [
      ...topics,
    ...
    const activated = this.associativeNetwork.activateMultiple(
      allLabels, 0.6 as Normalized, tick
    );
  ...
  private emotionToLabel(emotion: EmotionType): string {
    const map: Partial<Record<EmotionType, string>> = {
  ...
  private buildGuideline(
    analysis: { intent: ConversationIntent; emotions: EmotionType[]; hasSelfDisclosure: boolean },
    depth: ConversationDepth,
    ...
    if (hasNameIntro && visitor?.name) {
      specialInstructions.push(
        `訪問者が名前を教えてくれた（${visitor.name}）。名前を覚えたことを自然に伝える`
      );
    }
    ...
    // 応答の深さ
    let responseDepth: 'brief' | 'normal' | 'detailed' = 'normal';
   ...
   * 自律発話用の分離プロンプトを構築
   */
  buildAutonomousSpeechSeparatedPrompt(
    provider: SelfModelStateProvider,
    spontaneousSeed: string,
    tick: Tick
  ...
  tolegacyLLMContext(ctx: RichResponseContext, userMessage: string): LLMContext {
    return {
      currentEmotion: ctx.self.currentMood.includes('寂しい') ? 'loneliness' as EmotionType :
                      ctx.self.currentMood.includes('穏やか') ? 'peace' as EmotionType :
                      ctx.self.currentMood.includes('嬉しい') ? 'joy' as EmotionType :
                      'peace' as EmotionType,
      emotionalIntensity: 0.5 as Normalized,
  ...
  validateResponse(
    response: string,
    ctx: RichResponseContext,
    userMessage: string
  ): { isValid: boolean; issues: string[]; suggestion: string | null } {
    const issues: string[] = [];
  ...
  onVisitorArrival(currentAction: { action: string } | null): void {
  ...
  fromJSON(data: any): void {
    if (!data) return;
    if (data.recentResponses) this.recentResponses = data.recentResponses;
  }
}
