...
export interface PatternLibraryConfig {
  ...
  pruneThreshold: number;  // 使用回数がこれ以下で古いものは削除候補
}
...
export class PatternLibrary {
  private speechPatterns: Map<ID, SpeechPattern> = new Map();
  private behaviorPatterns: Map<ID, BehaviorPattern> = new Map();
  private emotionPatterns: Map<ID, EmotionPattern> = new Map();
   ...
   * 初期発話パターン
   */
  private initializeSpeechPatterns(): void {
    const initialPatterns: Omit<SpeechPattern, 'id' | 'createdAt' | 'useCount' | 'lastUsed' | 'mutationHistory'>[] = [
      // 感情表現パターン
      {
  ...
  private initializeBehaviorPatterns(): void {
    const initialPatterns: Omit<BehaviorPattern, 'id' | 'createdAt' | 'lastUsed' | 'mutationHistory'>[] = [
      // 読書パターン
      {
        actionSequence: [
  ...
  private initializeEmotionPatterns(): void {
    const initialPatterns: Omit<EmotionPattern, 'id' | 'modificationHistory'>[] = [
      // 朝の感情
      {
        situation: {
   ...
   * 状況に合う発話パターンを検索
   */
  findMatchingSpeechPatterns(
    emotionalState: Partial<EmotionalState>,
    timeOfDay: TimeOfDay,
    activity: ActionType | null,
    visitorPresent: boolean,
    urges: Record<string, Normalized>
  ): SpeechPattern[] {
    const matches: { pattern: SpeechPattern; score: number }[] = [];
   ...
   * 状況に合う行動パターンを検索
   */
  findMatchingBehaviorPatterns(
    urges: Record<string, Normalized>,
    timeOfDay: TimeOfDay,
    emotionalState: Partial<EmotionalState>,
    currentEnergy: Normalized
  ): BehaviorPattern[] {
    const matches: { pattern: BehaviorPattern; score: number }[] = [];
    ...
    for (const pattern of this.behaviorPatterns.values()) {
      ...
      for (const trigger of pattern.triggers) {
        ...
        switch (trigger.type) {
          case 'urge_threshold':
            const urgeKey = trigger.condition.urge as string;
          ...
          case 'emotional':
            if (emotionalState.primary === trigger.condition.emotion &&
                (emotionalState.levels?.[trigger.condition.emotion as EmotionType] || 0) >= trigger.condition.intensity) {
              score += trigger.probability;
            }
            break;
          case 'random':
   ...
   * 状況に合う感情パターンを検索
   */
  findMatchingEmotionPatterns(
    timeOfDay: TimeOfDay,
    urges: Record<string, Normalized>,
    visitorPresent: boolean,
    recentEvents: string[]
  ): EmotionPattern[] {
    const matches: EmotionPattern[] = [];
   ...
   * 発話パターンを変異
   */
  private mutateSpeechPattern(patternId: ID): boolean {
    const pattern = this.speechPatterns.get(patternId);
    if (!pattern || !pattern.canMutate) return false;
    
    // 変異確率チェック
    if (Math.random() > this.config.mutationRate) return false;
    
    // 変異タイプを選択
    const mutationType = this.selectMutationType();
    
    let mutation: PatternMutation | null = null;
   ...
   * 行動パターンを変異
   */
  private mutateBehaviorPattern(patternId: ID): boolean {
   ...
   * 感情パターンを変異
   */
  private mutateEmotionPattern(patternId: ID): boolean {
    const pattern = this.emotionPatterns.get(patternId);
    if (!pattern || !pattern.canModify) return false;
  ...
  evolvePatterns(): PatternEvolutionResult {
    ...
    for (const { patternId, fitness, diagnosis } of behaviorFitness) {
      const pattern = this.behaviorPatterns.get(patternId);
      if (!pattern || !pattern.canMutate) continue;
   ...
   * 行動パターンの適応度を評価
   */
  private evaluateBehaviorFitness(): Array<{ patternId: ID; fitness: number; diagnosis: string }> {
    const results: Array<{ patternId: ID; fitness: number; diagnosis: string }> = [];
    
    for (const [id, pattern] of this.behaviorPatterns) {
      const totalUses = pattern.successCount + pattern.failureCount;
      if (totalUses < 3) continue; // データ不足
      ...
      let diagnosis = '';
      if (successRate < 0.3) diagnosis = '成功率が低い';
      else if (satisfaction < 0.3) diagnosis = '満足度が低い';
      else if (recencyScore < 0.2) diagnosis = '長期間使われていない';
      else diagnosis = '適応度良好';
   ...
   * 大幅な行動パターン変異
   */
  private radicalBehaviorMutation(pattern: BehaviorPattern): boolean {
    const mutations: (() => boolean)[] = [
      // 行動シーケンスの一部を新しいものに入れ替え
      () => {
        if (pattern.actionSequence.length < 2) return false;
        const idx = Math.floor(Math.random() * pattern.actionSequence.length);
        const newActions: ActionType[] = ['wander', 'rest', 'think', 'look_at', 'examine', 'write', 'sing', 'daydream'];
      ...
      // トリガーの確率を変更
      () => {
        if (pattern.triggers.length === 0) return false;
   ...
   * 統合候補のペアを探す
   */
  private findMergeCandidates(): Array<{ patternA: ID; patternB: ID; similarity: number }> {
    const candidates: Array<{ patternA: ID; patternB: ID; similarity: number }> = [];
    ...
    for (let i = 0; i < behaviorList.length; i++) {
      for (let j = i + 1; j < behaviorList.length; j++) {
        ...
        // トリガーの類似度で判定
        const triggersA = new Set(patternA.triggers.map((t: PatternTrigger) => t.type));
        const triggersB = new Set(patternB.triggers.map((t: PatternTrigger) => t.type));
   ...
   * 2つのパターンを統合
   */
  private mergePatterns(idA: ID, idB: ID): ID | null {
   ...
   * パターンを2つに分岐
   */
  private splitBehaviorPattern(patternId: ID): [ID, ID] | null {
    const pattern = this.behaviorPatterns.get(patternId);
    if (!pattern || pattern.triggers.length < 2 || pattern.actionSequence.length < 3) return null;
   ...
   * 発話パターンを感情状態に適応させる
   */
  private adaptSpeechToEmotion(pattern: SpeechPattern): boolean {
    // テンプレートの語尾を感情に合わせて変化
   ...
   * 感情パターンの感度を経験に基づいて調整
   */
  private adjustEmotionSensitivity(pattern: EmotionPattern): boolean {
    if (Math.random() > 0.2) return false; // 20%の確率でのみ調整
   ...
   * 弱いパターンを淘汰
   */
  private eliminateWeakPatterns(): Array<{ patternId: ID; type: string; description: string }> {
    const eliminated: Array<{ patternId: ID; type: string; description: string }> = [];
    const now = Date.now();
    const thirtyDaysAgo = now - 30 * 24 * 60 * 60 * 1000;
    
    // 行動パターン
    for (const [id, pattern] of this.behaviorPatterns) {
      if (pattern.createdBy === 'initial') continue;
  ...
  private matchEmotionCondition(condition: PatternCondition, emotionalState: Partial<EmotionalState>): boolean {
    const value = emotionalState.levels?.[condition.value as EmotionType] || 0;
  ...
  private matchTimeCondition(condition: PatternCondition, timeOfDay: TimeOfDay): boolean {
  ...
  private matchUrgeCondition(condition: PatternCondition, urges: Record<string, Normalized>): boolean {
  ...
  private selectMutationType(): 'minor_variation' | 'combination' | 'simplification' | 'expansion' {
    const rand = Math.random();
    if (rand < 0.4) return 'minor_variation';
    if (rand < 0.6) return 'simplification';
    if (rand < 0.9) return 'expansion';
    return 'combination';
  }
  
  private createMinorVariation(pattern: SpeechPattern): PatternMutation | null {
    // シンプルなテンプレート変異
  ...
  private generateExampleVariation(pattern: SpeechPattern): string | null {
  ...
  private pruneOldPatterns(type: 'speech' | 'behavior' | 'emotion'): void {
    const now = Date.now();
    const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000;
    
    let patterns: Map<ID, any>;
    let maxPatterns: number;
    ...
    if (patterns.size <= maxPatterns) return;
    
    // 削除候補を見つける
    const candidates: { id: ID; score: number }[] = [];
  ...
  getStats(): {
    ...
    mostUsedSpeechPatterns: { id: ID; useCount: number }[];
    mostSuccessfulBehaviorPatterns: { id: ID; successRate: number }[];
  } {
