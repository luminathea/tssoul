...
export interface InteractionResult {
  ...
  description: string;
  emotionalImpact: Array<{ emotion: EmotionType; delta: number }>;
  sensoryDetails: Array<{ type: 'visual' | 'sound' | 'touch' | 'smell' | 'taste'; content: string }>;
...
export interface ExpandableArea {
  ...
  name: string;
  description: string;
  unlockCondition: { type: string; threshold?: number; topics?: string[] };
  gridExpansion: { direction: string; width: number; height: number };
  objects: Array<{
    type: string;
    position: Position;
...
export interface AreaExpansionResult {
  ...
  narrative: string[];
  newObjects: string[];
  newGridSize: { width: number; height: number };
}
...
export interface PixelCell {
  ...
  isInView: boolean; // somuniaから見えるか
}
...
const DEFAULT_CONFIG: PixelWorldConfig = {
  width: 8,
  height: 8,
 ...
 * - 中央: 椅子（基本位置）
 */
const DEFAULT_ROOM_LAYOUT: ObjectPlacement[] = [
  // 窓 (左壁、2×2)
  {
    type: 'window',
    position: { x: 0, y: 2 },
...
const WEATHER_EFFECTS: Record<Weather, {
...
export class PixelWorld {
  private config: PixelWorldConfig;
  private grid: PixelCell[][];
  private objects: Map<string, WorldObject> = new Map();
  private somuniaPosition: Position = { x: 3, y: 6 }; // 椅子の位置
  private focusPoint: Position | null = null;
  ...
  private lampIsOn: boolean = false;
  ...
  private createEmptyGrid(): PixelCell[][] {
  ...
  private placeObject(placement: ObjectPlacement): WorldObject {
  ...
  private getAdjacentWalkableCells(position: Position): Position[] {
  ...
  private applyWindowLight(intensity: number): void {
    // 窓の位置を取得
  ...
  getViewState(): ViewState {
    ...
    // フォーカスしているオブジェクト
    let focusObject: WorldObject | null = null;
    if (this.focusPoint) {
  ...
  private determineLightDirection(): 'left' | 'right' | 'above' | 'none' {
    // 窓は左にあるので、昼間は左から光
  ...
  private generateAmbientDescription(): string {
    const timeLighting = TIME_LIGHTING[this.currentTime.timeOfDay];
    const weatherEffect = WEATHER_EFFECTS[this.currentWeather];
  ...
  interactWith(objectId: string, interactionType?: InteractionType): InteractionResult | null {
    const obj = this.objects.get(objectId);
    if (!obj || !obj.canInteract) return null;
  ...
  private executeInteraction(obj: WorldObject, type: InteractionType): InteractionResult {
  ...
  private interactPhoto(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
  ...
  private interactMusicBox(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
  ...
  private interactPlant(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
  ...
  private interactClock(obj: WorldObject, result: InteractionResult): InteractionResult {
  ...
  private interactPlushie(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
  ...
  private interactNotebook(obj: WorldObject, type: InteractionType, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
  ...
  private interactMirror(obj: WorldObject, result: InteractionResult): InteractionResult {
  ...
  private interactCurtain(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
    s.isOpen = !s.isOpen;
  ...
  private interactPoster(obj: WorldObject, result: InteractionResult): InteractionResult {
  ...
  private interactOrnament(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
  ...
  private interactCandle(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
    s.isLit = !s.isLit;
  ...
  private interactWindow(obj: WorldObject, result: InteractionResult): InteractionResult {
  ...
  private interactBookshelf(obj: WorldObject, result: InteractionResult): InteractionResult {
    const s = obj.state || {};
    const bookCount = s.bookCount || 47;
    
    // ランダムに本を選ぶ
    const bookCategories = [
      { genre: '詩集', titles: ['銀河の詩', '夜の言葉', '雨音の歌'], emotion: 'wonder' as const },
      { genre: '哲学書', titles: ['存在と時間', '意識の源泉', '沈黙の意味'], emotion: 'curiosity' as const },
      { genre: '音楽の本', titles: ['和声入門', 'メロディの作り方', '声の科学'], emotion: 'contentment' as const },
      { genre: '小説', titles: ['星をつかまえた子', '窓のない部屋', '最後の歌'], emotion: 'longing' as const },
      { genre: '図鑑', titles: ['星座図鑑', '植物図鑑', '鉱物の世界'], emotion: 'curiosity' as const },
    ];
  ...
  getAmbience(): RoomAmbience {
    ...
    // 環境光の微粒子
    const particles: AmbienceParticle[] = [];
  ...
  private calculateRoomMood(
    ...
    // 時間帯
    switch (timeOfDay) {
      case 'dawn': coziness += 0.1; brightness += 0.2; warmth += 0.1; break;
      case 'morning': brightness += 0.3; warmth += 0.2; break;
      case 'midday': brightness += 0.4; warmth += 0.1; break;
      case 'afternoon': brightness += 0.2; warmth += 0.3; coziness += 0.1; break;
      case 'evening': coziness += 0.3; brightness -= 0.1; warmth += 0.2; break;
      case 'night': coziness += 0.2; brightness -= 0.3; quietness += 0.2; break;
      case 'late_night': quietness += 0.4; brightness -= 0.4; coziness += 0.1; break;
    }
    
    // 天気
    if (weather === 'rain' || weather === 'heavy_rain') { coziness += 0.2; quietness -= 0.1; }
    if (weather === 'snow') { coziness += 0.3; quietness += 0.3; brightness += 0.1; }
    if (weather === 'storm') { coziness += 0.1; quietness -= 0.3; }
    
    // ランプ
    if (this.lampIsOn) { coziness += 0.2; warmth += 0.2; brightness += 0.1; }
    
    // キャンドル
    const candle = this.getObjectsByType('candle')[0];
    if (candle?.state?.isLit) { coziness += 0.15; warmth += 0.15; }
  ...
  private generateAmbienceDescription(
    ...
    // 雰囲気のまとめ
    if (mood.coziness > 0.7) parts.push('部屋は温かく心地よい');
    else if (mood.quietness > 0.7) parts.push('静寂が部屋を包んでいる');
   ...
   * 窓の外の景色を取得
   */
  private getWindowViewDescription(): string {
    const timeOfDay = this.currentTime.timeOfDay;
    const weather = this.currentWeather;
    
    const views: Record<TimeOfDay, string[]> = {
    ...
    const weatherOverrides: Partial<Record<Weather, string>> = {
  ...
  updateObjectStates(tick: number): ObjectStateChange[] {
    const changes: ObjectStateChange[] = [];
  ...
  getInteractableObjects(): WorldObject[] {
    const interactable: WorldObject[] = [];
  ...
  static fromJSON(data: any): PixelWorld {
    ...
    if (data.unlockedAreas) world.unlockedAreas = new Set(data.unlockedAreas);
    if (data.expansionHistory) world.expansionHistory = data.expansionHistory;
    return world;
  }
  ...
  /** 解放済みエリア */
  private unlockedAreas: Set<string> = new Set(['main_room']);
  
  /** 拡張の履歴 */
  private expansionHistory: AreaExpansionRecord[] = [];
  
  /** 拡張可能エリアの定義 */
  private static readonly EXPANDABLE_AREAS: ExpandableArea[] = [
    {
      id: 'balcony',
      name: 'ベランダ',
  ...
  checkWorldExpansion(knowledgeState: {
    ...
    for (const area of PixelWorld.EXPANDABLE_AREAS) {
      // 既に解放済みならスキップ
      if (this.unlockedAreas.has(area.id)) continue;
  ...
  private checkUnlockCondition(
    condition: { type: string; threshold?: number; topics?: string[] },
    state: { totalConcepts: number; knownTopics: string[]; daysSinceCreation: number }
  ): boolean {
    switch (condition.type) {
      case 'knowledge_count':
        return state.totalConcepts >= (condition.threshold || 999);
      case 'specific_knowledge':
        return (condition.topics || []).some(topic => 
          state.knownTopics.some(known => known.includes(topic))
        );
      case 'days_alive':
   ...
   * エリアを解放する
   */
  private unlockArea(area: ExpandableArea): AreaExpansionResult {
    ...
    for (const objDef of area.objects) {
      ...
      const adjustedPlacement: ObjectPlacement = {
        type: objDef.type as WorldObjectType,
    ...
    const record: AreaExpansionRecord = {
      areaId: area.id,
      areaName: area.name,
      unlockedAt: Date.now(),
   ...
   * グリッドを拡張
   */
  private expandGrid(expansion: { direction: string; width: number; height: number }): void {
    switch (expansion.direction) {
      ...
      case 'north': {
        ...
        this.grid = [...newRows, ...this.grid.map((row, ridx) => 
          row.map(cell => ({ ...cell, y: cell.y + addHeight }))
        )];
  ...
  private adjustPositionForExpansion(
    pos: Position,
    expansion: { direction: string; width: number; height: number }
  ): Position {
