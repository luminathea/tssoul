...
import { PixelWorld, ViewState, PixelCell } from './PixelWorld';
...
export interface Percept {
  id: string;
  type: PerceptType;
  source: WorldObject | string; // オブジェクトまたは環境
  position?: Position;
  salience: number; // 0-1: 注意を引く度合い
  emotionalValence: number; // -1 to 1: 感情的価値
  description: string;
  timestamp: number;
}

/** 知覚の種類 */
export type PerceptType =
  | 'visual' // 視覚
  | 'auditory' // 聴覚
  | 'temporal' // 時間感覚
  | 'atmospheric' // 雰囲気
...
export interface AttentionState {
  focus: Percept | null; // 現在の注意対象
  peripheral: Percept[]; // 周辺的に知覚しているもの
  distractions: Percept[]; // 気が散る要素
  attentionLevel: number; // 0-1: 集中度
  wanderingThoughts: string[]; // 思考の漂い
}

/** 環境の印象 */
export interface EnvironmentalImpression {
  overall: string; // 全体的な印象
  mood: EmotionType; // 環境が誘発する気分
  comfort: number; // 0-1: 心地よさ
  novelty: number; // 0-1: 新鮮さ
  safety: number; // 0-1: 安全感
  details: string[]; // 気づいた詳細
}

/** 知覚のフィルター設定 */
export interface PerceptionFilter {
  emotionalBias: Partial<Record<EmotionType, number>>; // 感情による知覚の偏り
  interestAreas: WorldObjectType[]; // 興味のある対象
  avoidanceAreas: WorldObjectType[]; // 避けたい対象
  attentionThreshold: number; // 注意を向ける閾値
}
...
export interface PerceptionConfig {
  maxPercepts: number; // 同時に知覚できる最大数
  attentionDecayRate: number; // 注意の減衰率
  salienceThreshold: number; // 注意を引く最低閾値
  thoughtWanderingRate: number; // 思考が漂う確率
}
...
const OBJECT_PERCEPTION: Record<WorldObjectType, {
  baseSalience: number;
  emotionalValence: number;
  timeModifiers: Partial<Record<TimeOfDay, number>>;
  descriptions: string[];
  poeticDescriptions: string[];
}> = {
  window: {
    baseSalience: 0.7,
...
export class Perception {
  ...
  private currentPercepts: Map<string, Percept> = new Map();
  private attentionState: AttentionState;
  private perceptionEvents: PerceptionEvent[] = [];
  private filter: PerceptionFilter;
  
  // 知覚の履歴
  private recentPercepts: Percept[] = [];
  private maxRecentPercepts = 50;
  ...
  private perceiveVisuals(
    ...
    if (viewState.focusObject) {
      const focusPercept = percepts.find(
        p => p.source === viewState.focusObject
      );
      if (focusPercept) {
  ...
  private createObjectPercept(obj: WorldObject, time: WorldTime): Percept {
  ...
  private createLightPercept(direction: string, time: WorldTime): Percept {
    const descriptions: Record<string, Record<TimeOfDay, string>> = {
      left: {
  ...
  private createWeatherPercept(weather: Weather, time: WorldTime): Percept | null {
    if (weather === 'clear') return null;
    
    const weatherDescriptions: Record<Weather, {
      visual: string;
      sound?: string;
      salience: number;
      valence: number;
  ...
  private perceiveTime(time: WorldTime): Percept {
  ...
  private perceiveInternal(emotions: EmotionState): Percept | null {
    // 支配的な感情を見つける
    let dominantEmotion: EmotionType = 'serenity';
    let maxValue = 0;
    ...
    const internalDescriptions: Partial<Record<EmotionType, string[]>> = {
  ...
  private updateAttention(percepts: Percept[]): void {
    // 最も顕著なものにフォーカス
    ...
    // 注意レベルの減衰
    this.attentionState.attentionLevel = Math.max(
      0.3,
      this.attentionState.attentionLevel - this.config.attentionDecayRate
    );
  ...
  private generateWanderingThought(percepts: Percept[]): string | null {
    const thoughts = [
      '...そういえば',
      '...なぜだろう',
      '...いつか',
  ...
  getEnvironmentalImpression(world: PixelWorld): EnvironmentalImpression {
    ...
    // 環境が誘発する気分
    const mood: EmotionType = this.determineMoodFromEnvironment(time, weather, lighting);
  ...
  private calculateComfort(lighting: LightingState, weather: Weather): number {
    let comfort = 0.7;
  ...
  private gatherEnvironmentDetails(world: PixelWorld): string[] {
  ...
  private updateFilterByEmotions(emotions: EmotionState): void {
  ...
  private updateCurrentPercepts(percepts: Percept[]): void {
