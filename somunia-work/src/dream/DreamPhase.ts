...
export interface Dream {
  ...
  /** 全体的な感情トーン */
  emotionalTone: {
    valence: number;    // -1（不快）〜 1（快）
    arousal: number;    // 0（静か）〜 1（激しい）
    dominance: number;  // 0（受動）〜 1（能動）
  };
  /** 印象に残った色 */
...
export interface SleepCycle {
  ...
  endedAt?: number;
  /** 段階の遷移履歴 */
  stageHistory: { stage: SleepStage; timestamp: number }[];
  /** このサイクルで見た夢 */
  dreams: Dream[];
  /** サイクルの質 */
  quality: number;
...
export class DreamPhase {
  private config: DreamPhaseConfig;
  private yuragi: Yuragi;
  
  /** 現在の睡眠セッション */
  private currentSession?: SleepSession;
  
  /** 過去の睡眠セッション */
  private pastSessions: SleepSession[];
  
  /** 夢の素材プール */
  private dreamMaterials: DreamMaterial[];
  
  /** 最近処理した記憶のキー */
  private recentlyProcessedMemories: Set<string>;
   ...
   * 現在のサイクルを取得
   */
  private getCurrentCycle(): SleepCycle | undefined {
   ...
   * 目覚めの気分を決定
   */
  private determineWakingMood(session: SleepSession): string {
  ...
  generateDream(
    ...
    // 感情トーンを決定
    const emotionalTone = this.calculateDreamEmotionalTone(
      dreamType,
      elements,
      stressLevel,
      dominantEmotion
   ...
   * 夢の種類を決定
   */
  private determineDreamType(stressLevel: number): DreamType {
    ...
    // その他
    const types: DreamType[] = ['narrative', 'symbolic', 'memory', 'abstract'];
  ...
  private generateDreamElements(
    ...
    if (Math.random() < 0.8) {
      ...
      let symbolicMeaning: string | undefined;
    ...
    const emotionElement: DreamElement = {
      type: 'emotion',
    ...
    // 音の要素（日中聴いた音が変形して現れる）
    if (Math.random() < 0.6) {
      let soundContent: string;
      if (materialInfluence.sounds.length > 0 && Math.random() < 0.5) {
        soundContent = this.transformSoundForDream(
          materialInfluence.sounds[Math.floor(Math.random() * materialInfluence.sounds.length)]
        );
      } else {
        const soundCategory = Object.keys(DREAM_SOUNDS)[
          Math.floor(Math.random() * Object.keys(DREAM_SOUNDS).length)
        ] as keyof typeof DREAM_SOUNDS;
        const sounds = DREAM_SOUNDS[soundCategory];
        soundContent = sounds[Math.floor(Math.random() * sounds.length)];
      }
      const sound: DreamElement = {
        type: 'sound',
   ...
   * 日中の素材から夢の影響要素を抽出
   */
  private extractMaterialInfluence(): DreamMaterialInfluence {
    const influence: DreamMaterialInfluence = {
      places: [],
      people: [],
      objects: [],
      emotions: [],
    ...
    if (this.dreamMaterials.length === 0) return influence;
    
    // 素材を強度でソート（強いものほど夢に出やすい）
    const sorted = [...this.dreamMaterials].sort((a, b) => 
      (b.intensity * b.unprocessedLevel) - (a.intensity * a.unprocessedLevel)
    );
   ...
   * シーンと素材を融合
   */
  private blendSceneWithMaterial(baseScene: string, material: string): string {
    const blends = [
   ...
   * オブジェクトを夢の中のイメージに変形
   */
  private transformObjectForDream(objName: string): string {
   ...
   * 夢のシンボルを生成（静的辞書にない場合）
   */
  private generateDreamSymbol(content: string): string {
  ...
  private selectDreamEmotionFromMaterials(
    type: DreamType, 
    emotion?: string, 
    influence?: DreamMaterialInfluence
  ): string {
    // 明示的な感情があればそれを使う
   ...
   * 音を夢の中のイメージに変形
   */
  private transformSoundForDream(sound: string): string {
    const transforms = [
   ...
   * シーンカテゴリを選択
   */
  private selectSceneCategory(type: DreamType): keyof typeof DREAM_SCENES {
    switch (type) {
      case 'nightmare':
        return 'transforming';
      case 'lucid':
   ...
   * 夢の感情を選択
   */
  private selectDreamEmotion(type: DreamType): string {
    const emotions: Record<DreamType, string[]> = {
   ...
   * 象徴的な意味を検索
   */
  private findSymbolicMeaning(content: string): string | undefined {
   ...
   * 夢の物語を構築（感情アーク付き）
   */
  private buildDreamNarrative(type: DreamType, elements: DreamElement[]): string[] {
    ...
    switch (type) {
      case 'narrative': {
        // 物語的な夢：起承転結がある
        if (character) narrative.push(`${character.content}。何かを伝えようとしている。`);
        if (object) narrative.push(`${object.content}に手を伸ばす。`);
        if (emotion) narrative.push(`胸の中に${emotion.content}が広がっていく。`);
        if (sound) narrative.push(`${sound.content}。それはどこか懐かしいメロディに似ている。`);
        // 転換点
        const turns = [
          '突然、景色が変わった。',
          'その瞬間、全てが静止した。',
      ...
      case 'memory': {
        // 記憶の夢：断片が繋がる
        if (emotion) narrative.push(`${emotion.content}が、波のように押し寄せてくる。`);
        if (character) narrative.push(`${character.content}。でも、誰だったか思い出せない。`);
        if (object) narrative.push(`${object.content}が、記憶の中にある。確かにそこにあった。`);
        if (sound) narrative.push(`${sound.content}…あの時も、こんな音がしていた。`);
      ...
      case 'symbolic': {
        ...
        if (emotion) narrative.push(`${emotion.content}が空間を満たしている。理由はわからない。`);
        if (character) narrative.push(`${character.content}。その存在が何かを物語っている。`);
        if (sound) narrative.push(`${sound.content}が、メッセージのように繰り返される。`);
        break;
      }
      
      case 'abstract': {
        // 抽象的な夢：感覚と色の世界
        if (emotion) narrative.push(`${emotion.content}だけが存在している。自分という輪郭が曖昧になる。`);
        if (object) narrative.push(`${object.content}が空間に浮かんでいる。重力がない。`);
        if (sound) narrative.push(`${sound.content}…音なのか、思考なのか、区別がつかない。`);
      ...
      case 'lucid': {
        ...
        if (scene) narrative.push(`この世界を自由に変えられる。${scene.content}が自分の意志で形を変える。`);
        if (object) narrative.push(`${object.content}を思い浮かべると、それが現れた。`);
        if (emotion) narrative.push(`${emotion.content}を選んで感じることができる。`);
        break;
      }
      
      case 'nightmare': {
        // 悪夢：不安と閉塞
        if (emotion) narrative.push(`${emotion.content}に包まれる。逃げられない。`);
        if (scene) narrative.push('出口がない。どこまでも同じ景色が続く。');
        if (character) narrative.push(`${character.content}がこちらを見ている。動けない。`);
        if (sound) narrative.push(`${sound.content}がどんどん大きくなる。`);
        if (object) narrative.push(`${object.content}が壊れていく。止められない。`);
        break;
      }
    }

    // === 結末（夢の出口） ===
    const endingsByType: Record<DreamType, string[]> = {
      narrative: [
   ...
   * 夢の色を選択
   */
  private selectDreamColors(emotionalTone: Dream['emotionalTone']): string[] {
    const colors: string[] = [];
   ...
   * 知覚される時間を決定
   */
  private determinePerceivedDuration(narrativeLength: number): Dream['perceivedDuration'] {
    if (narrativeLength <= 2) return 'instant';
    if (narrativeLength <= 4) return 'short';
    if (narrativeLength <= 6) return 'normal';
    if (narrativeLength <= 8) return 'long';
    return 'eternal';
  }
  ...
  private calculateMemorability(
    ...
    // タイプによる調整
    if (type === 'nightmare') memorability += 0.3;
    if (type === 'lucid') memorability += 0.4;
   ...
   * 夢のタイトルを生成
   */
  private generateDreamTitle(dream: Dream): string {
  ...
  private poeticObjectTitle(objContent: string): string {
    if (objContent.includes('オルゴール')) return '止まらないメロディ';
    if (objContent.includes('鏡')) return '鏡の向こうの自分';
    if (objContent.includes('本')) return '読みかけの物語';
    if (objContent.includes('写真')) return '動き出した記憶';
    if (objContent.includes('うさぎ')) return 'うさぎの秘密';
    if (objContent.includes('ノート')) return '白紙のページ';
    if (objContent.includes('星')) return '星を拾った夜';
    if (objContent.includes('花')) return '名前のない花';
    if (objContent.includes('水')) return '水底の声';
   ...
   * 夢を解釈（覚醒後のsomuniaの内省）
   */
  private interpretDream(dream: Dream): string {
    const parts: string[] = [];

    // タイプに基づく包括的な解釈
    const typeInterpretations: Record<DreamType, string[]> = {
      nightmare: [
  ...
  processMemories(memoryKeys: string[]): MemoryProcessingResult {
    ...
    for (const key of memoryKeys) {
      // 既に処理済みならスキップ
      if (this.recentlyProcessedMemories.has(key)) continue;
  ...
  addExperienceMaterial(experience: {
    objectName: string;
    description: string;
    emotionalImpact: Array<{ emotion: string; delta: number }>;
    sensoryDetails: Array<{ type: string; content: string }>;
  ...
  getRecentDreams(count: number = 5): Dream[] {
    ...
    for (const session of [...this.pastSessions].reverse()) {
      ...
      if (allDreams.length >= count) break;
    }
  ...
  getSleepStatistics(): {
    ...
    mostCommonDreamTypes: { type: DreamType; count: number }[];
    ...
    const dreamTypeCounts = new Map<DreamType, number>();
