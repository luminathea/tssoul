...
interface BehaviorInsight {
  ...
  emotionAfterCounts: Record<string, number>;
  contexts: Array<{
    urgeLevel: { type: string; level: number };
    energyLevel: number;
    emotionPrimary: EmotionType;
    satisfaction: number;
  }>;
}
...
const BEHAVIOR_RULES: ActionRule[] = [
  // === 睡眠関連 ===
  {
    id: 'sleep_tired',
    name: '疲れて眠る',
...
export class BehaviorEngine {
  ...
  private currentAction: ActionInProgress | null = null;
  private recentActions: ActionType[] = [];
  private maxRecentActions: number = 20;
  private actionHistory: Array<{ rule: string; action: ActionType; timestamp: Tick; }> = [];
  private customRules: ActionRule[] = [];
  ...
  decideAction(
    ...
    yuragiLevel: Normalized = 0,
    yuragiOverride?: ActionType
  ): {
    rule: ActionRule;
   ...
   * 思考テンプレートから選択
   */
  private selectThought(rule: ActionRule): string {
    if (rule.thoughtTemplates.length === 0) return '';
  ...
  fromJSON(data: any): void {
    if (data.currentAction) this.currentAction = data.currentAction;
    if (data.recentActions) this.recentActions = data.recentActions;
    if (data.actionHistory) this.actionHistory = data.actionHistory;
    if (data.discoveryLog) this.discoveryLog = data.discoveryLog;
    if (data.behaviorInsights) this.behaviorInsights = new Map(Object.entries(data.behaviorInsights));
  ...
  /** 発見ログ */
  private discoveryLog: BehaviorDiscovery[] = [];
  ...
  recordActionOutcome(
    ...
    insight.attempts++;
    if (outcome.success) insight.successes++;
    ...
    if (insight.contexts.length > 10) insight.contexts.shift();
   ...
   * 経験から新しい行動ルールを発見する
   */
  private tryDiscoverNewRule(insight: BehaviorInsight, latestContext: BehaviorContext): void {
    ...
    if (successRate > 0.7 && avgSatisfaction > 0.6) {
      const existingTimeRule = this.customRules.find(
        r => r.action === insight.action && r.id.includes(`time_${insight.timeOfDay}`)
      );
    ...
    if (emotionPattern) {
      const existingEmotionRule = this.customRules.find(
        r => r.id.includes(`emo_${emotionPattern.trigger}_${insight.action}`)
      );
    ...
    if (energyPattern) {
      const existingEnergyRule = this.customRules.find(
        r => r.id.includes(`energy_${insight.action}`)
      );
   ...
   * 時間帯に特化したルールを生成
   */
  private createTimeSpecificRule(insight: BehaviorInsight): BehaviorDiscovery | null {
    const timeOfDay = insight.timeOfDay;
    const action = insight.action;
    
    // 既存ルールで同じ行動・時間帯のものがあるか確認
    const existingRule = this.rules.find(
      r => r.action === action && r.id.includes(timeOfDay)
    );
  ...
  private createEmotionTriggeredRule(
    ...
    pattern: { trigger: EmotionType; resultEmotion: EmotionType; frequency: number }
  ): BehaviorDiscovery | null {
    const action = insight.action;
  ...
  private createEnergyOptimizedRule(
    insight: BehaviorInsight,
    pattern: { optimalEnergyRange: [number, number]; avgSatisfaction: number }
  ): BehaviorDiscovery | null {
    const action = insight.action;
   ...
   * 行動連鎖ルールの発見
   */
  private tryDiscoverSequenceRule(insight: BehaviorInsight): void {
    // 履歴から連続して成功した行動ペアを探す
    if (this.actionHistory.length < 10) return;
    
    const sequenceCounts = new Map<string, { count: number; satisfaction: number }>();
    ...
    for (const [key, data] of sequenceCounts) {
      if (data.count >= 3) {
        const [prevAction, currAction] = key.split('→') as [ActionType, ActionType];
        const existingSequence = this.customRules.find(
          r => r.id.includes(`seq_${prevAction}_${currAction}`)
        );
  ...
  private findEmotionActionPattern(insight: BehaviorInsight): {
    trigger: EmotionType;
    resultEmotion: EmotionType;
    frequency: number;
  } | null {
    // 最も多い行動後の感情を見つける
    let bestResult: EmotionType | null = null;
    let bestCount = 0;
    for (const [emotion, count] of Object.entries(insight.emotionAfterCounts)) {
      if (count > bestCount) {
        bestCount = count;
        bestResult = emotion as EmotionType;
      }
    }
    
    if (!bestResult || bestCount < 3) return null;
    
    // 良い結果の場合のみ
    const positiveEmotions: EmotionType[] = ['joy', 'contentment', 'serenity', 'wonder', 'gratitude'];
    if (!positiveEmotions.includes(bestResult)) return null;
  ...
  private findEnergyPattern(insight: BehaviorInsight): {
    optimalEnergyRange: [number, number];
    avgSatisfaction: number;
  } | null {
    if (insight.contexts.length < 5) return null;
    
    // 満足度の高い文脈のエネルギーレベルを分析
    const highSat = insight.contexts.filter(c => c.satisfaction > 0.6);
    if (highSat.length < 3) return null;
    ...
    // 既存ルールと大差ない場合はスキップ
    if (maxEnergy - minEnergy > 0.6) return null;
  ...
  private getTopUrge(context: BehaviorContext): { type: string; level: number } {
    let topType = 'none';
    let topLevel = 0;
  ...
  private inferSatisfiedUrges(action: ActionType): UrgeType[] {
    // 既存ルールから同じアクションのsatisfiesを推定
    const existingRule = this.rules.find(r => r.action === action);
    if (existingRule) return existingRule.satisfies;
    
    // フォールバック推定
    const urgeMap: Record<string, UrgeType[]> = {
  ...
  private actionToJapanese(action: ActionType): string {
  ...
  private emotionToJapanese(emotion: EmotionType): string {
    const map: Record<string, string> = {
   ...
   * メタデータからカスタムルールを再構築
   */
  private regenerateCustomRule(saved: any): void {
    const meta = saved._meta;
    if (!meta) return;
    
    let condition: ((ctx: BehaviorContext) => boolean) | null = null;
    ...
    switch (meta.conditionType) {
      ...
      case 'emotion_triggered':
        condition = (ctx) => ctx.emotion.levels[meta.triggerEmotion as EmotionType] > 0.4 && ctx.body.energy > 0.2;
        break;
   ...
   * 行動インサイトのサマリー
   */
  getInsightSummary(): Array<{ action: ActionType; timeOfDay: TimeOfDay; successRate: number; avgSatisfaction: number }> {
    const results: Array<{ action: ActionType; timeOfDay: TimeOfDay; successRate: number; avgSatisfaction: number }> = [];
