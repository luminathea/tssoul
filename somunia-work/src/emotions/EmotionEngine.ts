...
export interface EmotionEngineConfig {
  // 感情の減衰速度（0-1、大きいほど速く減衰）
  decayRate: number;
  
  // 感情の慣性維持（0-1、大きいほど慣性が強い）
...
interface EmotionTriggerStat {
  ...
  subsequentEmotions: Record<string, number>;
}
 ...
 * 負の値: 相互抑制
 */
const EMOTION_RELATIONS: Partial<Record<EmotionType, Partial<Record<EmotionType, number>>>> = {
  joy: {
    peace: 0.3,
    contentment: 0.5,
    warmth: 0.4,
...
const TIME_EMOTION_BIAS: Record<TimeOfDay, Partial<Record<EmotionType, number>>> = {
  dawn: {
    peace: 0.15,
    hope: 0.1,
    wonder: 0.1,
...
export type EmotionTrigger = 
  | { type: 'pattern'; patternId: ID }
  | { type: 'decay' }
  | { type: 'time'; timeOfDay: TimeOfDay }
  | { type: 'event'; eventName: string }
  | { type: 'visitor'; event: 'arrived' | 'departed' | 'message' }
  | { type: 'action'; action: string; outcome: 'started' | 'completed' | 'failed' }
  | { type: 'memory'; memoryId: ID }
  | { type: 'yuragi'; yuragiType: string }
  | { type: 'urge'; urgeType: string }
  | { type: 'homeostasis'; state: string }
...
export class EmotionEngine {
  ...
  private changeHistory: EmotionChangeEvent[];
  ...
  private createInitialState(): EmotionalState {
    ...
    const levels = {} as Record<EmotionType, Normalized>;
    const momentum = {} as Record<EmotionType, number>;
  ...
  update(currentTick: Tick, timeOfDay: TimeOfDay): EmotionChangeEvent | null {
    ...
    if (changes.length > 0) {
      const event: EmotionChangeEvent = {
        timestamp: Date.now(),
        trigger: { type: 'decay' },
  ...
  changeEmotion(
    ...
    const clampedDelta = Math.max(
      -this.config.maxChangePerTick,
      Math.min(this.config.maxChangePerTick, delta)
    );
   ...
   */
  changeEmotions(
    changes: Array<{ emotion: EmotionType; delta: number }>,
    trigger: EmotionTrigger
  ): EmotionChangeEvent {
    const allChanges: EmotionChange[] = [];
    
    for (const { emotion, delta } of changes) {
      const previousLevel = this.state.levels[emotion];
      const clampedDelta = Math.max(
        -this.config.maxChangePerTick * 2,  // 複数変化時は許容幅を増やす
  ...
  applyPattern(pattern: EmotionPattern): EmotionChangeEvent {
    ...
    const changes: Array<{ emotion: EmotionType; delta: number }> = [
  ...
  respondToVisitor(event: 'arrived' | 'departed' | 'message', familiarity: Normalized): EmotionChangeEvent {
    ...
    switch (event) {
      case 'arrived':
        changes.push(
          { emotion: 'warmth', delta: 0.2 + familiarity * 0.2 },
          { emotion: 'loneliness', delta: -0.3 },
          { emotion: 'anticipation', delta: 0.15 }
        );
        if (familiarity > 0.5) {
      ...
      case 'departed':
        changes.push(
          { emotion: 'loneliness', delta: 0.15 + familiarity * 0.1 },
          { emotion: 'melancholy', delta: 0.1 },
          { emotion: 'warmth', delta: -0.2 }
        );
        if (familiarity > 0.7) {
  ...
  respondToAction(
    ...
    if (outcome === 'completed') {
      changes.push(
        { emotion: 'contentment', delta: satisfaction * 0.2 },
        { emotion: 'joy', delta: satisfaction * 0.1 }
      );
      ...
      if (action.includes('rest') || action.includes('sleep')) {
        changes.push(
          { emotion: 'peace', delta: 0.2 },
          { emotion: 'fatigue', delta: -0.3 }
        );
      }
    } else if (outcome === 'failed') {
      changes.push(
        { emotion: 'confusion', delta: 0.1 },
        { emotion: 'anxiety', delta: 0.05 }
      );
   ...
   * 自然な減衰を適用
   */
  private applyDecay(changes: EmotionChange[]): void {
    const baselineEmotions: Partial<Record<EmotionType, Normalized>> = {
      peace: 0.3,      // somuniaは基本的に穏やか
      melancholy: 0.1, // 少しの物思い
      curiosity: 0.2   // 緩やかな好奇心
    };
    
    for (const emotion of Object.keys(this.state.levels) as EmotionType[]) {
   ...
   * 慣性による変化を適用
   */
  private applyMomentum(changes: EmotionChange[]): void {
    for (const emotion of Object.keys(this.state.momentum) as EmotionType[]) {
      const momentum = this.state.momentum[emotion];
      
      if (Math.abs(momentum) > 0.001) {
        const previousLevel = this.state.levels[emotion];
        this.state.levels[emotion] = Math.max(
          0,
          Math.min(1, previousLevel + momentum * 0.1)
        );
   ...
   * 時間帯バイアスを適用
   */
  private applyTimeBias(timeOfDay: TimeOfDay, changes: EmotionChange[]): void {
    ...
    for (const [emotion, amount] of Object.entries(bias)) {
      ...
      this.state.levels[emotionType] = Math.max(
        0,
        Math.min(1, previousLevel + delta)
      );
   ...
   * 感情間の相互作用を適用
   */
  private applyEmotionInteractions(changes: EmotionChange[]): void {
    const interactions: Array<{ emotion: EmotionType; delta: number }> = [];
    
    for (const emotion of Object.keys(EMOTION_RELATIONS) as EmotionType[]) {
      const relations = EMOTION_RELATIONS[emotion];
      if (!relations) continue;
      
      const sourceLevel = this.state.levels[emotion];
      if (sourceLevel < this.config.activationThreshold) continue;
      
      for (const [targetEmotion, strength] of Object.entries(relations)) {
        const delta = sourceLevel * (strength as number) * this.config.emotionInteractionStrength * 0.01;
        if (Math.abs(delta) > 0.001) {
          interactions.push({
            emotion: targetEmotion as EmotionType,
            delta
          });
        }
      }
   ...
   * 感情変化を他の感情に伝播
   */
  private propagateInteraction(sourceEmotion: EmotionType, delta: number): EmotionChange[] {
    const changes: EmotionChange[] = [];
    ...
    for (const [targetEmotion, strength] of Object.entries(relations)) {
      const propagatedDelta = delta * (strength as number) * this.config.emotionInteractionStrength;
      if (Math.abs(propagatedDelta) > 0.01) {
        const previousLevel = this.state.levels[targetEmotion as EmotionType];
        this.state.levels[targetEmotion as EmotionType] = Math.max(
          0,
          Math.min(1, previousLevel + propagatedDelta)
        );
        
        changes.push({
          emotion: targetEmotion as EmotionType,
          previousLevel,
          newLevel: this.state.levels[targetEmotion as EmotionType],
   ...
   * valence/arousalを更新
   */
  private updateValenceArousal(): void {
    // Valence: ポジティブ - ネガティブ
    const positive = (
      this.state.levels.joy +
      this.state.levels.peace +
      this.state.levels.contentment +
      this.state.levels.warmth +
      this.state.levels.hope
    ) / 5;
   ...
   * マッチするパターンを検索
   */
  private findMatchingPattern(situation: SituationDescriptor): EmotionPattern | null {
  ...
  private scoreSituationMatch(
    patternSituation: SituationDescriptor,
    currentSituation: SituationDescriptor
  ): number {
    let score = 0;
    let factors = 0;
    ...
    if (patternSituation.timeOfDay && currentSituation.timeOfDay) {
      factors++;
      if (patternSituation.timeOfDay.some(t => 
        currentSituation.timeOfDay?.includes(t)
      )) {
        score += 1;
      }
    }
   ...
   * トリガーを文字列に変換
   */
  private triggerToString(trigger: EmotionTrigger): string {
    switch (trigger.type) {
   ...
   * アクティブな感情（閾値以上）を取得
   */
  getActiveEmotions(): Array<{ emotion: EmotionType; level: Normalized }> {
  ...
  private generateDescription(): string {
    ...
    const emotionWords: Record<EmotionType, string> = {
      joy: '嬉しい',
      peace: '穏やか',
      curiosity: '興味深い',
   ...
   * 最近の変化履歴を取得
   */
  getRecentChanges(count: number = 10): EmotionChangeEvent[] {
  ...
  /** 感情トリガーの統計 */
  private triggerStats: Map<string, EmotionTriggerStat> = new Map();
  
  /** 感情の滞留検知用 */
  private stagnationTracker: Map<EmotionType, { level: number; duration: number }> = new Map();
  
  /** 自己修正の履歴 */
  private selfCorrectionLog: EmotionSelfCorrection[] = [];
  ...
  recordTriggerOutcome(
    ...
    intensity: number,
    outcome: {
      wasHelpful: boolean;        // この感情反応は適切だったか
      durationTicks: number;      // 持続ティック数
      ledToAction: boolean;       // 行動に繋がったか
      subsequentEmotion?: EmotionType; // 後続の感情
    }
  ): void {
    ...
    stat.occurrences++;
    if (outcome.wasHelpful) stat.helpfulCount++;
    stat.totalIntensity += intensity;
    stat.totalDuration += outcome.durationTicks;
    if (outcome.ledToAction) stat.actionLeadCount++;
  ...
  performSelfCorrection(): EmotionSelfCorrection[] {
    const corrections: EmotionSelfCorrection[] = [];
    
    // === 1. 過敏反応の鈍化 ===
    for (const [key, stat] of this.triggerStats) {
      if (stat.occurrences < 5) continue;
    ...
    if (diversityCorrection) corrections.push(diversityCorrection);
    ...
    if (baselineCorrection) corrections.push(baselineCorrection);
   ...
   * 過敏な感情を鈍化
   */
  private desensitizeEmotion(emotion: EmotionType, currentAvgIntensity: number): EmotionSelfCorrection | null {
   ...
   * 行動に繋がらない感情を弱める
   */
  private reduceIdleEmotion(stat: EmotionTriggerStat): EmotionSelfCorrection | null {
   ...
   * 滞留状態を解消
   */
  private breakStagnation(emotion: EmotionType, tracker: { level: number; duration: number }): EmotionSelfCorrection | null {
   ...
   * 感情の多様性を確保
   */
  private ensureEmotionalDiversity(): EmotionSelfCorrection | null {
    ...
    if (activeEmotions.length <= 1 && this.changeHistory.length >= 20) {
      ...
      const uniqueEmotions = new Set(last20.map(c => {
        // 最大変化の感情を代表とする
        const maxChange = c.changes?.reduce((max, ch) => 
          ch.newLevel > (max?.newLevel || 0) ? ch : max, c.changes[0]);
        return maxChange?.emotion || this.state.primary;
      }));
      
      if (uniqueEmotions.size <= 2) {
        // somuniaらしい感情を少し活性化
        const dormantEmotions: EmotionType[] = ['curiosity', 'wonder', 'nostalgia'];
   ...
   * 感情間の結合を経験から調整
   */
  private adjustEmotionCouplings(): EmotionSelfCorrection[] {
    const corrections: EmotionSelfCorrection[] = [];
    
    // 後続感情のパターンから結合強度を学習
    for (const [, stat] of this.triggerStats) {
      if (stat.occurrences < 8) continue;
      ...
      for (const [subsequent, count] of Object.entries(stat.subsequentEmotions)) {
        ...
        if (frequency > 0.5) {
          ...
          const relations = EMOTION_RELATIONS[emotion];
          if (relations) {
            const currentCoupling = relations[subsequent as EmotionType] || 0;
            ...
            if (Math.abs(newCoupling - currentCoupling) > 0.01) {
              ...
              if (!this.couplingOverrides) this.couplingOverrides = new Map();
   ...
   */
  private adaptBaselineEmotions(): EmotionSelfCorrection | null {
    if (this.changeHistory.length < 50) return null;
    
    // 最近50件の平均レベルを計算
    const recentAvg: Partial<Record<EmotionType, number>> = {};
    const emotions = Object.keys(this.state.levels) as EmotionType[];
    ...
    for (const [emotion, avg] of Object.entries(recentAvg)) {
      const current = this.state.levels[emotion as EmotionType];
      if (current !== undefined && Math.abs((avg as number) - current) > 0.15) {
        // 平均に向かって微調整
        const direction = (avg as number) > current ? 0.02 : -0.02;
        this.state.momentum[emotion as EmotionType] = 
          (this.state.momentum[emotion as EmotionType] || 0) + direction;
        adjusted = true;
  ...
  /** 結合強度のオーバーライド */
  private couplingOverrides: Map<string, number> | null = null;
   ...
   * 自己修正ログを取得
   */
  getSelfCorrectionLog(count: number = 20): EmotionSelfCorrection[] {
