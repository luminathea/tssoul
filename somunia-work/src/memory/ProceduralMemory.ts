...
export type SkillCategory =
  | 'expression'     // 表現系（歌う、書く）
  | 'physical'       // 身体系（歩く、座る）
  | 'cognitive'      // 認知系（読む、考える）
  | 'social'         // 社会系（話す、聞く）
  | 'creative'       // 創造系（作曲、詩作）
  | 'technical';     // 技術系（PC操作）
...
interface SkillDefinition {
  ...
  baseComplexity: Normalized;  // 基本的な難しさ
}
...
export class ProceduralMemorySystem {
  private skills: Map<ID, ProceduralMemory>;
  private config: ProceduralMemoryConfig;
  private eventLog: SkillExecutionEvent[];
  private lastMaintenanceTick: Tick;
  
  // インデックス
  private skillNameIndex: Map<string, ID>;
  private categoryIndex: Map<SkillCategory, Set<ID>>;
  private actionIndex: Map<ActionType, Set<ID>>;
   ...
   * スキルを実行
   */
  private executeSkill(skill: ProceduralMemory): SkillExecution {
    // 自動化されていれば簡単に成功
   ...
   * 学習の転移
   */
  private transferLearning(skill: ProceduralMemory): void {
    // 同じカテゴリのスキルを見つける
  ...
  performMaintenance(currentTick: Tick): void {
    ...
    if (ticksSinceLastMaintenance < 300) return;  // 300tick毎
   ...
   * スキルを削除
   */
  private deleteSkill(skillId: ID): void {
  ...
  private enforceCapacity(): void {
    if (this.skills.size <= this.config.maxSkills) return;
    ...
    const sorted = Array.from(this.skills.values())
      ...
      .sort((a, b) => 
        (a.proficiency * a.practiceCount) - (b.proficiency * b.practiceCount)
      );
   ...
   * 習熟度の高いスキルを取得
   */
  getMostProficientSkills(limit: number = 10): ProceduralMemory[] {
   ...
   * 練習が必要なスキルを取得
   */
  getSkillsNeedingPractice(limit: number = 5): ProceduralMemory[] {
    return Array.from(this.skills.values())
   ...
   * 最近のイベントを取得
   */
  getRecentEvents(count: number = 20): SkillExecutionEvent[] {
