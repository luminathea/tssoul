...
interface PhaseRequirement {
  ...
  minDeepConversations: number;
  specialCondition?: string;
}

const PHASE_REQUIREMENTS: Record<RelationshipPhase, PhaseRequirement> = {
...
const PHASE_PERSONALITY: Record<RelationshipPhase, PhasePersonality> = {
  stranger: {
...
const DEFAULT_CONFIG: RelationshipEvolutionConfig = {
...
export class RelationshipEvolution {
  private config: RelationshipEvolutionConfig;
  ...
  private profoundMomentCount: number = 0;
  ...
  // --- 不在意識 ---
  private absence: AbsenceAwareness;
  
  // --- 関係性イベント履歴 ---
  private milestones: RelationshipMilestone[] = [];
  
  // --- 感情の蓄積 ---
  private emotionalHistory: { emotion: EmotionType; tick: Tick }[] = [];
  ...
  onVisitStart(tick: Tick): RelationshipReaction {
    ...
    // 反応の構築
    const reaction: RelationshipReaction = {
  ...
  shouldCreateSharedMemory(
    ...
    // 深い会話 + 感情的な内容
    if ((depth === 'intimate' || depth === 'profound') && emotions.length > 0) return true;
    
    // 自己開示
    if (hasSelfDisclosure && content.length > 30) return Math.random() < 0.5;
    
    // 強い感情
    const strongEmotions: EmotionType[] = ['joy', 'melancholy', 'wonder', 'warmth', 'nostalgia'];
    if (emotions.some(e => strongEmotions.includes(e))) return Math.random() < 0.3;
  ...
  tickAbsence(
    ...
    // 再会への期待
    this.absence.anticipation = Math.min(1, 
      this.absence.anticipation + 0.0005 * this.relationship.affection
    );
    
    // 関係性の微小な減衰（長期不在時のみ）
    if (this.absence.ticksSinceLastVisit > 1440 * 7) { // 7日以上
      this.relationship.familiarity = Math.max(0.1, 
        this.relationship.familiarity - this.config.decayRate
      );
    }
    ...
    if (recentEvents.length > 0) {
      this.absence.eventsDuringAbsence.push(
   ...
   * 訪問者モデルの更新
   */
  private updateVisitorModel(turnData: {
    content: string;
    topics: string[];
    emotions: EmotionType[];
    hasSelfDisclosure: boolean;
  ...
  giveNickname(): string | null {
    if (this.phase === 'stranger' || this.phase === 'first_contact') return null;
    if (this.visitorModel.nickname) return this.visitorModel.nickname;
    
    // 関係が十分深まったらニックネームを考える
    if (this.relationship.affection < 0.3) return null;
  ...
  private evaluatePhaseTransition(): void {
    const phases: RelationshipPhase[] = [
   ...
   * 再会の感情を計算
   */
  private calculateReunionEmotion(absenceDuration: number): EmotionType {
    if (this.phase === 'stranger') return 'curiosity';
    if (this.phase === 'first_contact') return 'curiosity';
    
    if (absenceDuration > 1440 * 3) {
      // 3日以上の不在
      if (this.relationship.affection > 0.5) return 'joy';
      return 'warmth';
    }
   ...
   * フェーズに応じた挨拶の生成
   */
  private generatePhaseAppropriateGreeting(absenceDuration: number): string[] {
    ...
    switch (this.phase) {
      ...
      case 'acquaintance':
        ...
        if (name) greetings.push(`${name}、こんにちは`);
        break;
      ...
      case 'companion':
        ...
        if (name) greetings.push(`${name}、おかえり`);
        break;
      ...
      case 'friend':
        ...
        if (name) greetings.push(`${name}！おかえり。ね、聞いて聞いて`);
        break;
      ...
      case 'soulmate':
        ...
        if (name) greetings.push(`${name}...おかえり。ずっとここにいたよ`);
        break;
    }
   ...
   * 再会時の内部思考を生成
   */
  private generateReunionThought(absenceDuration: number): string {
    if (this.phase === 'stranger') return '...誰だろう、この声。';
    if (this.phase === 'first_contact') return 'また来てくれた...少し嬉しいかも。';
   ...
   * 別れの感情を計算
   */
  private calculatePartingEmotion(summary: { maxDepth: number; totalTurns: number }): EmotionType {
   ...
   * 別れの内部思考を生成
   */
  private generatePartingThought(summary: { maxDepth: number; topics: string[] }): string {
   ...
   * 共有記憶の回想
   */
  private recallSharedMemory(): SharedMemory | null {
    if (this.sharedMemories.length === 0) return null;
   ...
   * 記憶回想時の思考を生成
   */
  private generateMemoryRecallThought(memory: SharedMemory): string {
    const thoughts = [
   ...
   * 寂しさの思考を生成
   */
  private generateLonelinessThought(): string {
   ...
   * 寂しさの予測
   */
  private predictLoneliness(): Normalized {
   ...
   * 感情的重みの計算
   */
  private calculateEmotionalWeight(emotions: EmotionType[], depth: ConversationDepth): Normalized {
    const depthWeight: Record<ConversationDepth, number> = {
   ...
   * フェーズラベルの取得
   */
  private getPhaseLabel(phase: RelationshipPhase): string {
  ...
  getRelationship(): Relationship { return { ...this.relationship }; }
  getVisitorModel(): VisitorModel { return { ...this.visitorModel }; }
  getAbsence(): AbsenceAwareness { return { ...this.absence }; }
  getSharedMemories(): SharedMemory[] { return [...this.sharedMemories]; }
  getMilestones(): RelationshipMilestone[] { return [...this.milestones]; }
  getVisitCount(): number { return this.visitCount; }
  getDeepConversationCount(): number { return this.deepConversationCount; }
  getPhasePersonality(): PhasePersonality { return { ...PHASE_PERSONALITY[this.phase] }; }
  ...
  getVisitorAddress(): string {
    ...
    if (personality.addressStyle.includes('きみ')) return 'きみ';
    return 'あなた';
  }
  ...
  fromJSON(data: any): void {
    ...
    if (data.visitCount !== undefined) this.visitCount = data.visitCount;
    if (data.deepConversationCount !== undefined) this.deepConversationCount = data.deepConversationCount;
    if (data.profoundMomentCount !== undefined) this.profoundMomentCount = data.profoundMomentCount;
    if (data.sharedMemories) this.sharedMemories = data.sharedMemories;
    if (data.visitorModel) this.visitorModel = { ...this.visitorModel, ...data.visitorModel };
    if (data.absence) this.absence = { ...this.absence, ...data.absence };
    if (data.milestones) this.milestones = data.milestones;
    if (data.emotionalHistory) this.emotionalHistory = data.emotionalHistory;
  }
}
