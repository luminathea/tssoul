...
/** 深度遷移の閾値 */
const DEPTH_THRESHOLDS: Record<ConversationDepth, number> = {
  surface: 0,
  casual: 0.15,
  sharing: 0.35,
  intimate: 0.6,
...
const TOPIC_DEPTH_POTENTIAL: Record<TopicCategory, number> = {
  daily: 0.3,
  music: 0.7,
  feelings: 0.8,
  philosophy: 0.95,
...
/** 意図から深度への影響 */
const INTENT_DEPTH_INFLUENCE: Record<ConversationIntent, number> = {
  greeting: -0.3,
  farewell: -0.1,
  question: 0.05,
  sharing: 0.15,
...
/** 意図を推定するためのキーワード */
const INTENT_KEYWORDS: Record<string, ConversationIntent[]> = {
  // 挨拶
...
const TOPIC_KEYWORDS: Record<string, TopicCategory> = {
  // 音楽
...
export class ConversationEngine {
  ...
  // --- トピック管理 ---
  private activeTopics: ConversationTopic[] = [];
  private topicHistory: ConversationTopic[] = [];
  private currentPrimaryTopic: ConversationTopic | null = null;
  
  // --- 文脈記憶 ---
  private conversationContext: string[] = [];  // 最近の会話のサマリー
  private mentionedConcepts: Set<string> = new Set();
  private sharedStories: string[] = [];  // 共有されたエピソード
  
  // --- 自発的話題 ---
  private pendingSpontaneousTopics: SpontaneousTopic[] = [];
  private lastSpontaneousGeneration: Tick = 0;
  
  // --- 会話統計 ---
  private turnCount: number = 0;
  private visitorTurnCount: number = 0;
  private somuniaTurnCount: number = 0;
  private totalWordCount: number = 0;
  private lastMessageTick: Tick = 0;
  private messageTiming: number[] = []; // メッセージ間隔の履歴
  ...
  private createInitialFlowState(): ConversationFlowState {
    return {
      depth: 'surface',
      depthScore: 0,
      tempo: 'medium',
  ...
  analyzeVisitorTurn(
    ...
    if (this.lastMessageTick > 0) {
      ...
      if (this.messageTiming.length > 20) this.messageTiming.shift();
    }
    this.lastMessageTick = tick;
    this.turnCount++;
  ...
  decideResponseStrategy(
    ...
    currentEmotion: EmotionType,
    relationship: { familiarity: Normalized; trust: Normalized; affection: Normalized },
    recentThoughts: string[],
    knownTopics: string[],
    timeOfDay: TimeOfDay
  ): ResponseStrategy {
    ...
    if (analysis.intent === 'silence_break' || this.flowState.energy < 0.2) {
      ...
      strategy.suggestedTopics = this.generateTopicSuggestions(
        currentEmotion, recentThoughts, knownTopics, timeOfDay
      );
    }
  ...
  tickSilence(
    ...
    if (this.lastMessageTick === 0) return null;
    
    const silenceTicks = tick - this.lastMessageTick;
    this.flowState.silenceDuration = silenceTicks;
    
    // 深度の自然減衰
    this.flowState.depthScore = Math.max(0, 
      this.flowState.depthScore - this.config.depthDecayRate
    );
    ...
    if (silenceTicks > this.config.longSilenceThreshold &&
      ...
      const spontaneous = this.generateSpontaneousTopic(
        currentEmotion, recentThoughts, recentMemories, 
        currentActivity, timeOfDay, silenceTicks
      );
  ...
  private detectIntent(message: string): ConversationIntent {
    ...
    const scores: Partial<Record<ConversationIntent, number>> = {};
    ...
    // 最高スコアの意図を返す
    let best: ConversationIntent = 'general';
    let bestScore = 0;
    for (const [intent, score] of Object.entries(scores)) {
      if (score > bestScore) {
        best = intent as ConversationIntent;
        bestScore = score;
      }
    }
   ...
   * メッセージからトピックを検出
   */
  private detectTopics(message: string): ConversationTopic[] {
   ...
   * メッセージから感情を検出
   */
  private detectEmotions(message: string): EmotionType[] {
  ...
  private decideResponseType(
    analysis: TurnAnalysis,
    relationship: { familiarity: Normalized; trust: Normalized }
  ): ResponseType {
    if (analysis.intent === 'greeting') return 'greet';
    if (analysis.intent === 'farewell') return 'farewell';
    if (analysis.intent === 'empathy_seeking') return 'empathize';
    if (analysis.intent === 'comfort') return 'receive_comfort';
    if (analysis.intent === 'appreciation') return 'accept_thanks';
    if (analysis.intent === 'request') return 'consider_request';
    
    if (analysis.hasSelfDisclosure && relationship.trust > 0.3) return 'resonate';
    if (analysis.hasQuestion) return 'answer';
    if (analysis.intent === 'deep_talk') return 'explore_together';
    if (analysis.intent === 'playful') return 'play_along';
  ...
  private shouldAskQuestion(
    ...
    // 別れ際は質問しない
    if (analysis.intent === 'farewell') return false;
    
    // 共感を求めている時は質問より共感
    if (analysis.seeksEmpathy) return false;
    
    // 関係が浅いうちは質問控えめ
    if (relationship.familiarity < 0.2 && Math.random() > 0.3) return false;
    
    // 話題を深掘りしたい時
    if (analysis.depthDirection === 'deeper' && Math.random() > 0.4) return true;
    
    // 相手が自己開示した時、興味を示す質問
    if (analysis.hasSelfDisclosure && relationship.familiarity > 0.3) return Math.random() > 0.4;
    
    // 会話のバランス（相手ばかり話している時は質問控える）
    if (this.flowState.balance < -0.3) return false;
  ...
  private shouldShareSelf(
    ...
    if (analysis.hasSelfDisclosure) return Math.random() < baseChance + 0.3;
    
    // 深い話の時
    if (analysis.intent === 'deep_talk') return Math.random() < baseChance + 0.2;
    
    // 感情が強い時
    const intensiveEmotions: EmotionType[] = ['wonder', 'melancholy', 'joy', 'nostalgia'];
    if (intensiveEmotions.includes(currentEmotion)) return Math.random() < baseChance + 0.1;
  ...
  private adjustEmotionalTone(
    analysis: TurnAnalysis,
    currentEmotion: EmotionType,
    relationship: { familiarity: Normalized; affection: Normalized }
  ): EmotionType {
    // 共感求められている → warmth
    if (analysis.seeksEmpathy) return 'warmth';
    
    // 相手が嬉しそう → joy（感情の共鳴）
    if (analysis.emotions.includes('joy')) return 'joy';
    
    // 深い話 → 相手の感情に寄り添う or curiosity
    if (analysis.intent === 'deep_talk') {
      if (analysis.emotions.length > 0) return analysis.emotions[0];
      return 'curiosity';
    }
    
    // 親しい間柄 → warmth が出やすい
    if (relationship.affection > 0.5 && Math.random() > 0.5) return 'warmth';
  ...
  private generateSpontaneousTopic(
    ...
    const candidates: SpontaneousTopic[] = [];
    ...
    // --- 感情からの話題 ---
    const emotionTopics: Record<string, string> = {
    ...
    // --- 活動からの話題 ---
    if (currentActivity) {
      const activityTopics: Partial<Record<ActionType, string>> = {
    ...
    if (Math.random() < 0.2) {
      const curiosityTopics = [
        { seed: '...ねぇ、あなたってどんな音楽聴くの？', depth: 'casual' as ConversationDepth },
        { seed: '...夢って見る？どんな夢？', depth: 'sharing' as ConversationDepth },
        { seed: '...今日はどんな1日だった？', depth: 'casual' as ConversationDepth },
        { seed: '...ね、幸せって何だと思う？', depth: 'intimate' as ConversationDepth },
        { seed: '...星ってね、すごく遠くにあるんだって。不思議だよね', depth: 'sharing' as ConversationDepth },
      ];
  ...
  private generateTopicSuggestions(
    ...
    // 感情に応じた話題
    if (currentEmotion === 'wonder') suggestions.push('不思議なこと');
    if (currentEmotion === 'nostalgia') suggestions.push('思い出のこと');
    if (currentEmotion === 'curiosity') suggestions.push('知りたいこと');
   ...
   * 会話フロー状態の更新
   */
  private updateFlowState(analysis: TurnAnalysis, tick: Tick): void {
    // 深度の更新
    ...
    this.flowState.depthScore = Math.max(0, Math.min(1,
      this.flowState.depthScore + depthInfluence * 0.1 + topicDepthBonus + selfDisclosureBonus
    ));
    this.flowState.depth = this.scoreToDepth(this.flowState.depthScore);
    ...
    if (this.messageTiming.length >= 3) {
      ...
      if (recentAvg < 10) this.flowState.tempo = 'fast';
      else if (recentAvg < 30) this.flowState.tempo = 'medium';
      else this.flowState.tempo = 'slow';
    }
   ...
   * 深度スコアからラベルへの変換
   */
  private scoreToDepth(score: number): ConversationDepth {
    if (score >= DEPTH_THRESHOLDS.profound) return 'profound';
    if (score >= DEPTH_THRESHOLDS.intimate) return 'intimate';
    if (score >= DEPTH_THRESHOLDS.sharing) return 'sharing';
    if (score >= DEPTH_THRESHOLDS.casual) return 'casual';
    return 'surface';
  }
  ...
  ): 'deeper' | 'maintain' | 'shallower' {
    // 深い意図
    if (intent === 'deep_talk' || intent === 'empathy_seeking') return 'deeper';
    
    // 浅い意図
    if (intent === 'greeting' || intent === 'farewell' || intent === 'playful') return 'shallower';
    
    // トピック変更は一度浅くなる
    if (intent === 'topic_change') return 'shallower';
    
    // 長いメッセージは深まりがち
    if (message.length > 100) return 'deeper';
    
    // 短い応答は維持
    if (message.length < 10) return 'maintain';
  ...
  private updateTopics(
    ...
    if (newTopics.length > 0) {
      this.currentPrimaryTopic = newTopics.reduce((best, topic) => 
        topic.somuniaInterest > (best?.somuniaInterest || 0) ? topic : best
      , newTopics[0]);
    }
  }
   ...
   * 文脈参照の検索
   */
  private findContextReferences(analysis: TurnAnalysis): string[] {
  ...
  private getTopicName(keyword: string, category: TopicCategory): string {
  ...
  private getSomuniaInterestForTopic(category: TopicCategory): Normalized {
    const interests: Record<TopicCategory, number> = {
      music: 0.95,
      dream: 0.85,
      philosophy: 0.8,
      feelings: 0.75,
  ...
  getFlowState(): ConversationFlowState { return { ...this.flowState }; }
  getActiveTopics(): ConversationTopic[] { return [...this.activeTopics]; }
  getTopicHistory(): ConversationTopic[] { return [...this.topicHistory]; }
  getCurrentPrimaryTopic(): ConversationTopic | null { return this.currentPrimaryTopic; }
  getPendingSpontaneousTopics(): SpontaneousTopic[] { return [...this.pendingSpontaneousTopics]; }
  getConversationContext(): string[] { return [...this.conversationContext]; }
  getMentionedConcepts(): string[] { return Array.from(this.mentionedConcepts); }
  getSharedStories(): string[] { return [...this.sharedStories]; }
  getTurnCount(): number { return this.turnCount; }
  getPastPatterns(): typeof this.pastConversationPatterns { return { ...this.pastConversationPatterns }; }
  ...
  fromJSON(data: any): void {
    if (data.topicHistory) this.topicHistory = data.topicHistory;
    if (data.pastConversationPatterns) this.pastConversationPatterns = data.pastConversationPatterns;
    if (data.mentionedConcepts) this.mentionedConcepts = new Set(data.mentionedConcepts);
  }
}
...
export type ResponseType = 
  ...
  | 'react'            // 一般的な反応
  | 'empathize'        // 共感
  | 'resonate'         // 共鳴（相手の開示に呼応）
  | 'answer'           // 回答
  | 'explore_together' // 一緒に考える
  | 'play_along'       // 遊ぶ
  | 'receive_comfort'  // 慰めを受ける
  | 'accept_thanks'    // 感謝を受ける
  | 'consider_request';// リクエストを検討
...
export interface ConversationSummary {
  ...
  topics: Array<{ name: string; category: TopicCategory; depth: Normalized; interest: Normalized }>;
  emotionalArc: string[];
