...
const EMOTION_STYLE: Record<string, { color: string; icon: string }> = {
...
const TIME_DISPLAY: Record<TimeOfDay, { icon: string; label: string }> = {
  dawn:       { icon: '🌅', label: '夜明け' },
...
const DEFAULT_UI_CONFIG: TerminalUIV2Config = {
  mode: 'conversation',
...
export class TerminalUIV2 {
  private rl: readline.Interface | null = null;
  private events: EventBus;
  private config: TerminalUIV2Config;
  ...
  showThought(thought: ThoughtNode): void {
    ...
    if (this.config.mode === 'conversation') return; // 会話モードでは非表示
  ...
  showAction(action: ActionType, description?: string): void {
    ...
    if (this.config.mode === 'conversation') return;
  ...
  showEmotionChange(emotion: EmotionType, intensity: Normalized): void {
    if (!this.config.showEmotions) return;
    if (this.config.mode === 'conversation') return;
  ...
  showDiary(title: string, content: string): void {
    if (!this.config.showDreams) return;
  ...
  showLearning(topic: string, summary: string): void {
    if (!this.config.showLearning) return;
   ...
   */
  showDashboard(data: {
    modules: { name: string; status: string; detail: string }[];
    uptime: string;
    storage: string;
    tick: number;
    day: number;
  ...
  private handleCommand(input: string): void {
    ...
    switch (command) {
      ...
      case 'h':
        ...
        break;
      case 'mode':
        this.switchMode(args[0] as DisplayMode);
        break;
      case 'thoughts':
  ...
  private switchMode(mode: string | undefined): void {
    const validModes: DisplayMode[] = ['conversation', 'observer', 'dashboard', 'inspector'];
    if (!mode || !validModes.includes(mode as DisplayMode)) {
      this.showSystem(`使用法: /mode <${validModes.join('|')}>`);
      this.showSystem(`現在: ${this.config.mode}`);
      return;
    }
    this.config.mode = mode as DisplayMode;
    this.printModeInfo();
...
`);
  ...
  private setupEventListeners(): void {
    this.events.on('thought', (event) => {
      if (event.data?.thought) this.showThought(event.data.thought);
    });

    this.events.on('action_started', (event) => {
      if (event.data?.action) this.showAction(event.data.action);
    });
