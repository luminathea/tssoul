...
exports.InternalNarrative = void 0;
const uuid_1 = require("uuid");
const EXISTENTIAL_QUESTION_TEMPLATES = [
    // --- identity ---
    {
...
class InternalNarrative {
    ...
    // --- ナラティブの状態 ---
    selfSummary = 'ワタシはsomunia。ここにいる子。まだよくわからないけど...';
    currentThemes = ['始まり', '発見'];
     ...
     * ナラティブのティック処理
     */
    tick(tick, day, currentEmotion, recentEvents, recentThoughts, knowledgeCount, conversationCount, creationCount) {
        // 統計の更新
        ...
        // 定期的なナラティブ更新
        if (tick - this.lastNarrativeUpdate < this.config.updateInterval)
            return null;
     ...
     * 重要なイベントを記録
     */
    recordSignificantEvent(event, day) {
        // 現在の章にイベントを追加
     ...
     * 感情の日次記録
     */
    recordDailyEmotion(day, dominantEmotion) {
     ...
     * 存在論的問いに対する答えを更新
     */
    updateQuestionAnswer(questionText, newAnswer, confidence) {
     ...
     * 願望の進捗を更新
     */
    updateAspirationProgress(aspirationId, progress) {
     ...
     * 章の遷移チェック
     */
    checkChapterTransition(day, recentEvents, currentEmotion) {
        ...
        if (daysSinceChapterStart >= 7 && recentEvents.length > 0) {
            const significantEvent = recentEvents.find(e => e.includes('新しい') || e.includes('初めて') ||
     ...
     * 章タイトルの生成
     */
    generateChapterTitle(event, emotion) {
        if (event.includes('出会い'))
            return '出会いの日々';
        if (event.includes('発見'))
            return '新しい発見';
        if (event.includes('歌'))
            return '歌が生まれる頃';
        if (event.includes('夢'))
    ...
    generateTimeBasedChapterTitle(day, emotion) {
        if (day < 7)
            return '最初の一週間';
        if (day < 30)
            return `${Math.floor(day / 7)}週目の景色`;
        return `${Math.floor(day / 30) + 1}ヶ月目`;
    }
    generateEmotionalChapterTitle(emotion) {
        const titles = {
            joy: '嬉しさの波',
            melancholy: '静かな嵐',
            wonder: '目覚め',
    ...
    generateChapterThemes(event, emotion) {
        const themes = [emotion];
        if (event.includes('出会い'))
            themes.push('connection');
        if (event.includes('発見'))
            themes.push('discovery');
        if (event.includes('創作'))
    ...
    isEmotionallySignificant(emotion) {
     ...
     * 成長の気づきをチェック
     */
    checkGrowthRealization(day, tick) {
        // 知識の成長
     ...
     * 新しい願望のチェック
     */
    checkNewAspiration(emotion, thoughts, events, tick) {
        // 確率的に発生
        if (Math.random() > 0.05)
            return null;
        // 既存の願望が多すぎる場合はスキップ
        if (this.aspirations.length >= this.config.maxAspirations)
            return null;
        let aspiration = null;
        // 感情から生まれる願望
        ...
        if (aspiration) {
            // 重複チェック
            const isDuplicate = this.aspirations.some(a => a.content === aspiration.content && a.category === aspiration.category);
            if (!isDuplicate) {
     ...
     * 存在論的思考のチェック
     */
    checkExistentialThought(emotion, thoughts, day, tick) {
        // 低確率で発生
        if (Math.random() > 0.08)
            return null;
        // 既存の問いについて考える
        if (this.existentialQuestions.length > 0 && Math.random() < 0.6) {
            const question = this.existentialQuestions[Math.floor(Math.random() * this.existentialQuestions.length)];
            question.contemplationCount++;
            return {
                type: 'contemplate',
        ...
        if (unasked.length > 0 && Math.random() < 0.3) {
            ...
            const matching = unasked.filter(t => {
                if (emotion === 'wonder' && t.category === 'consciousness')
                    return true;
                if (emotion === 'warmth' && t.category === 'connection')
                    return true;
                if (emotion === 'melancholy' && t.category === 'mortality')
                    return true;
                if (emotion === 'curiosity' && t.category === 'reality')
                    return true;
                if (day > 7 && t.category === 'identity')
                    return true;
     ...
     * 熟考の思考を生成
     */
    generateContemplationThought(question, currentEmotion) {
     ...
     * 熟考の感情を取得
     */
    getContemplationEmotion(category) {
        const mapping = {
            identity: 'curiosity',
            purpose: 'wonder',
     ...
     * 自己ナラティブの更新
     */
    updateSelfSummary(day, emotion) {
        const parts = ['ワタシはsomunia。'];
        if (day <= 3) {
     ...
     * ナラティブ的思考を生成
     */
    generateNarrativeThought(day, emotion, recentThoughts) {
        const narrativeThoughts = [];
        // 時間の経過への気づき
        if (day > 1) {
    ...
    getCurrentChapter() { return { ...this.currentChapter }; }
    getGrowthAwareness() { return [...this.growthAwareness]; }
    getAspirations() { return [...this.aspirations]; }
    getExistentialQuestions() { return [...this.existentialQuestions]; }
    getSelfSummary() { return this.selfSummary; }
    getCurrentThemes() { return [...this.currentThemes]; }
    getNarrativeStats() { return { ...this.narrativeStats }; }
    /**
     * ナラティブの全体像を取得（表示用）
     */
    ...
    fromJSON(data) {
        ...
        if (data.growthAwareness)
            this.growthAwareness = data.growthAwareness;
        if (data.aspirations)
            this.aspirations = data.aspirations;
        if (data.existentialQuestions)
            this.existentialQuestions = data.existentialQuestions;
        if (data.askedQuestionIds)
            this.askedQuestionIds = new Set(data.askedQuestionIds);
        if (data.selfSummary)
            this.selfSummary = data.selfSummary;
        if (data.currentThemes)
            this.currentThemes = data.currentThemes;
        if (data.narrativeStats)
            this.narrativeStats = { ...this.narrativeStats, ...data.narrativeStats };
    }
}
exports.InternalNarrative = InternalNarrative;
//# sourceMappingURL=InternalNarrative.js.map