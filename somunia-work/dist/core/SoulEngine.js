...
exports.SoulEngine = void 0;
const EventBus_1 = require("./EventBus");
const TimeManager_1 = require("./TimeManager");
const LLMInterface_1 = require("../llm/LLMInterface");
const BehaviorEngine_1 = require("../behavior/BehaviorEngine");
const ThoughtEngine_1 = require("../mind/ThoughtEngine");
const Visitor_1 = require("../visitor/Visitor");
const PersistenceV3_1 = require("../persistence/PersistenceV3");
const TerminalUIV2_1 = require("../ui/TerminalUIV2");
// === 統合モジュール ===
...
const ProceduralMemory_1 = require("../memory/ProceduralMemory");
const PixelWorld_1 = require("../world/PixelWorld");
const Perception_1 = require("../world/Perception");
const HabitEngine_1 = require("../habits/HabitEngine");
const Diary_1 = require("../habits/Diary");
const DreamPhase_1 = require("../dream/DreamPhase");
const LearnEngine_1 = require("../knowledge/LearnEngine");
const WikipediaLearner_1 = require("../knowledge/WikipediaLearner");
const SelfModification_1 = require("../mind/SelfModification");
// === Phase 5: 対話深化と創造的表現 ===
const ConversationEngine_1 = require("../conversation/ConversationEngine");
const RelationshipEvolution_1 = require("../conversation/RelationshipEvolution");
const ConversationMemoryBridge_1 = require("../conversation/ConversationMemoryBridge");
const CreativeEngine_1 = require("../creative/CreativeEngine");
const InternalNarrative_1 = require("../narrative/InternalNarrative");
// === Phase 6: 外部接続・実運用 ===
const TestRunner_1 = require("../test/TestRunner");
// === Phase 7: 基盤刷新 ===
const AssociativeNetwork_1 = require("../association/AssociativeNetwork");
const SelfModel_1 = require("../self/SelfModel");
const VisitorMemorySystem_1 = require("../bridge/VisitorMemorySystem");
const ContextBridge_1 = require("../bridge/ContextBridge");
const PatternMemoryEngine_1 = require("../pattern/PatternMemoryEngine");
const GradualAutonomy_1 = require("../autonomy/GradualAutonomy");
// === Phase 7.5: Expression Fix ===
const ExpressionFilter_1 = require("../expression/ExpressionFilter");
// ============================================================
// SoulEngine
// ============================================================
class SoulEngine {
    ...
    reflectionState;
    // === タイミング設定 ===
    PERCEPTION_INTERVAL = 3; // 知覚更新間隔
    THOUGHT_INTERVAL = 5; // 思考生成間隔
    ACTION_INTERVAL = 10; // 行動決定間隔
    HABIT_CHECK_INTERVAL = 30; // 習慣チェック間隔
    STATUS_DISPLAY_INTERVAL = 30; // ステータス表示間隔
    REFLECTION_INTERVAL = 300; // 内省間隔（5分 = 300ティック）
    DIARY_WRITE_INTERVAL = 120; // 日記書き込み間隔
    MEMORY_MAINTENANCE_INTERVAL = 600; // 記憶メンテナンス間隔
    SLEEP_CYCLE_TICKS = 90; // 睡眠1サイクル（90ティック = 90分相当）
    DREAM_INTERVAL = 30; // 夢生成間隔
    // === Phase 5 タイミング ===
    CREATIVE_CHECK_INTERVAL = 60; // 創作衝動チェック間隔
    CREATIVE_PROGRESS_INTERVAL = 5; // 創作進行間隔
    NARRATIVE_INTERVAL = 720; // ナラティブ更新間隔（12時間相当）
    ABSENCE_TICK_INTERVAL = 30; // 不在感覚更新間隔
    AUTONOMOUS_SPEECH_INTERVAL = 20; // 自律発話チェック間隔
    lastSpeechTick = 0; // 最後に発話したtick
    RELATIONSHIP_TICK_INTERVAL = 60; // 関係性更新間隔
    // === Phase 7 タイミング ===
    AUTONOMY_EVAL_INTERVAL = 50; // 自律レベル評価間隔
    PATTERN_CULL_INTERVAL = 500; // パターン淘汰間隔
    ASSOCIATION_DECAY_INTERVAL = 100; // 連想減衰間隔
    constructor(config = {}) {
        this.config = config;
        const mc = config.modules || {};
        // === コアシステム初期化 ===
        this.events = EventBus_1.eventBus;
        this.time = new TimeManager_1.TimeManager(config.time, this.events);
        this.llm = new LLMInterface_1.LLMInterface(config.llm);
        // === 身体系初期化 ===
        this.homeostasis = new Homeostasis_1.Homeostasis(mc.homeostasis);
        this.urgeSystem = new UrgeSystem_1.UrgeSystem(mc.urges);
        // === 脳・揺らぎ初期化 ===
        this.patternLibrary = new PatternLibrary_1.PatternLibrary(mc.patterns);
        this.patternLibrary.initialize();
        this.yuragi = new Yuragi_1.YuragiSystem(mc.yuragi);
        // === 感情初期化 ===
        this.emotionEngine = new EmotionEngine_1.EmotionEngine(mc.emotions);
        // === 世界・知覚初期化 ===
        this.world = new PixelWorld_1.PixelWorld(mc.world);
        this.perception = new Perception_1.Perception(mc.perception);
        // === 思考・行動初期化 ===
        this.thoughts = new ThoughtEngine_1.ThoughtEngine(this.events);
        this.behavior = new BehaviorEngine_1.BehaviorEngine(this.events);
        // === 記憶初期化 ===
        this.episodicMemory = new EpisodicMemory_1.EpisodicMemorySystem(mc.episodicMemory);
        this.semanticMemory = new SemanticMemory_1.SemanticMemorySystem(mc.semanticMemory);
        this.proceduralMemory = new ProceduralMemory_1.ProceduralMemorySystem(mc.proceduralMemory);
        // === 習慣・日記初期化 ===
        this.habitEngine = new HabitEngine_1.HabitEngine(mc.habits);
        this.diary = new Diary_1.Diary(mc.diary);
        // === 夢初期化 ===
        this.dreamPhase = new DreamPhase_1.DreamPhase(mc.dream);
        // === 学習初期化 ===
        this.learnEngine = new LearnEngine_1.LearnEngine(mc.learn);
        this.wikipediaLearner = new WikipediaLearner_1.WikipediaLearner(mc.wikipedia);
        // === 自己変容初期化 ===
        this.selfModification = new SelfModification_1.SelfModification(mc.selfMod);
        // === 対話初期化 ===
        this.visitor = new Visitor_1.Visitor(this.events);
        // === Phase 5: 対話深化と創造的表現の初期化 ===
        this.conversationEngine = new ConversationEngine_1.ConversationEngine(mc.conversationEngine);
        this.relationshipEvolution = new RelationshipEvolution_1.RelationshipEvolution(mc.relationshipEvolution);
        this.conversationMemoryBridge = new ConversationMemoryBridge_1.ConversationMemoryBridge(mc.conversationMemoryBridge);
        this.creativeEngine = new CreativeEngine_1.CreativeEngine(mc.creativeEngine);
        this.internalNarrative = new InternalNarrative_1.InternalNarrative(mc.internalNarrative);
        // === Phase 7: 基盤刷新の初期化 ===
        this.associativeNetwork = new AssociativeNetwork_1.AssociativeNetwork(mc.associativeNetwork);
        this.selfModel = new SelfModel_1.SelfModel();
        this.visitorMemory = new VisitorMemorySystem_1.VisitorMemorySystem(mc.visitorMemory);
        this.contextBridge = new ContextBridge_1.ContextBridge(this.selfModel, this.visitorMemory, this.associativeNetwork);
        this.patternMemory = new PatternMemoryEngine_1.PatternMemoryEngine(mc.patternMemory);
        this.autonomy = new GradualAutonomy_1.GradualAutonomy(this.patternMemory, mc.autonomy);
        // === Phase 7.5: Expression Fix ===
        this.expressionFilter = new ExpressionFilter_1.ExpressionFilter();
        // === インフラ初期化 ===
        this.persistence = new PersistenceV3_1.PersistenceV3(config.persistence, this.events);
        if (!config.headless) {
            this.ui = new TerminalUIV2_1.TerminalUIV2({}, this.events);
        }
        // === 仮想身体の初期化 ===
    ...
    async mainLoop(tick) {
        ...
        if (this.visitor.isPresent() && tick % this.AUTONOMOUS_SPEECH_INTERVAL === 0 && !this.isProcessingMessage) {
            ...
            if (ticksSinceLastSpeech >= 40) {
                const spontaneous = this.conversationEngine.tickSilence(tick, this.emotionEngine.getState().primary, this.thoughts.getRecentThoughts(3).map(t => t.content), this.episodicMemory.getRecentMemories(3).map(m => m.summary), this.behavior.getCurrentAction()?.action || null, this.time.getTimeOfDay());
                if (spontaneous && this.ui) {
                    // Phase 7.5: 分離プロンプトで自然な自律発話を生成
                    const stateProvider = this.buildStateProvider();
                    const speechPrompt = this.contextBridge.buildAutonomousSpeechSeparatedPrompt(stateProvider, spontaneous.seed, tick);
                    let speechContent = await this.llm.expressAutonomousSpeech(speechPrompt.system, speechPrompt.user);
                    // フォールバック
        ...
        if (!this.visitor.isPresent() && tick % this.ABSENCE_TICK_INTERVAL === 0) {
            const absence = this.relationshipEvolution.tickAbsence(tick, this.emotionEngine.getState().primary, this.episodicMemory.getRecentMemories(2).map(m => m.summary));
            if (absence) {
                // 不在中の寂しさを感情に反映
                const loneliness = this.relationshipEvolution.getAbsence().loneliness;
                if (loneliness > 0.3) {
                    this.emotionEngine.changeEmotions([{ emotion: 'loneliness', delta: loneliness * 0.005 }], { type: 'event', eventName: 'visitor_absence' });
                }
            }
        }
        ...
        if (tick % this.AUTONOMY_EVAL_INTERVAL === 0 && tick > 0) {
            ...
            if (evalResult.levelChanged) {
                this.selfModel.recordExperience(`自律レベルが変わった: ${evalResult.previousLevel} → ${evalResult.currentLevel}`, tick);
            }
        }
        // === 23. Phase 7: パターン淘汰 ===
    ...
    processYuragi(tick, timeOfDay) {
        ...
        const result = this.yuragi.tick(tick, emotionalState, urgeState, timeOfDay, this.body.fatigue, currentAction?.action || null);
        // 揺らぎが発生した場合のイベント
        if (result.triggered) {
    ...
    // 3. 身体の更新
    // ============================================================
    updateBody(tick, timeOfDay) {
        // --- Homeostasis更新 ---
        const homeoEvents = this.homeostasis.update(tick, timeOfDay, this.body.fatigue);
    ...
    // 4. 感情の更新
    // ============================================================
    updateEmotions(tick, timeOfDay, yuragiResult) {
        ...
        if (yuragiChanges.length > 0) {
            this.emotionEngine.changeEmotions(yuragiChanges, { type: 'yuragi', yuragiType: 'fluctuation' });
        }
        // --- 一人の時間による寂しさ ---
        if (!this.visitor.isPresent()) {
            this.emotionEngine.changeEmotions([{ emotion: 'loneliness', delta: 0.00005 }], { type: 'event', eventName: 'alone_time' });
        }
        else {
            this.emotionEngine.changeEmotions([
    ...
    // 7. 思考の生成
    // ============================================================
    generateThought(tick, timeOfDay, timeState, currentAction, yuragiResult) {
        // 揺らぎによる思考侵入があれば優先
        ...
        if (thought) {
            const matching = this.patternLibrary.findMatchingSpeechPatterns(emotionalState, timeOfDay, currentAction?.action || null, this.visitor.isPresent(), this.getUrgeMap());
            // マッチするパターンがあれば使用記録
    ...
    // 9. 行動の決定
    // ============================================================
    decideAction(tick, timeOfDay, timeState, habitSuggestion, yuragiResult) {
        const urgeState = this.urgeSystem.getState();
        const emotionalState = this.emotionEngine.getState();
        // 揺らぎレベル
        ...
        const decision = this.behavior.decideAction(context, yuragiLevel, yuragiResult.actionOverride || undefined);
        if (decision) {
            ...
            // 行動開始による感情変化
            this.emotionEngine.changeEmotions([{ emotion: 'anticipation', delta: 0.05 }], { type: 'action', action: decision.rule.action, outcome: 'started' });
            // 学習行動なら学習セッションを開始
    ...
    onActionCompleted(action, tick) {
        ...
        switch (action) {
            ...
            case 'think':
            case 'reflect':
                this.emotionEngine.changeEmotions([{ emotion: 'peace', delta: 0.03 }], { type: 'action', action, outcome: 'completed' });
                break;
            case 'look_window':
                this.emotionEngine.changeEmotions([
                ...
                break;
            default:
                this.emotionEngine.changeEmotions([{ emotion: 'contentment', delta: 0.02 }], { type: 'action', action, outcome: 'completed' });
        }
        // --- 行動完了のエピソード記憶 ---
        this.episodicMemory.formMemory({
            content: `行動完了: ${action}`,
    ...
    // 11. 学習処理
    // ============================================================
    checkLearningStart(action, tick) {
        if (action === 'read_book') {
            this.learningState = {
                isLearning: true,
                type: 'book',
            ...
            this.wikipediaLearner.startExploration({
                ...
                intensity: this.urgeSystem.getUrgeLevel('curiosity'),
    ...
    progressLearning(tick) {
        ...
        if (elapsed % 20 === 0 && this.learningState.type === 'book') {
            ...
            const interest = this.urgeSystem.getUrgeLevel('curiosity');
            this.learnEngine.learn(`${topic}についての新しい知見`, 'book', 'fact', ['reading', topic], {
    ...
    // 12. 睡眠・夢処理
    // ============================================================
    checkSleepNeed(tick, timeOfDay, hour) {
        if (this.sleepState.isSleeping)
            return;
        const shouldSleep = (this.time.isSleepTime() && this.body.fatigue > 0.6) ||
    ...
    async processSleepTick(tick, timeOfDay, hour) {
    ...
    advanceSleepStage(tick, elapsed) {
        ...
        if (cyclePhase < 0.1)
            ...
            stage = 'light';
        this.dreamPhase.transitionToStage(stage);
    ...
    generateDream(tick) {
        ...
        const dream = this.dreamPhase.generateDream(emotionalState.levels.anxiety || 0.2, dominantEmotion, recentMemoryKeys);
        if (dream) {
            this.events.emitSync({
    ...
    async wakeUp(tick) {
        ...
        if (session?.rememberedDreams && session.rememberedDreams.length > 0) {
            const dreamSummaries = session.rememberedDreams
                .map((d) => d.title || '不思議な夢')
                .join('、');
            this.diary.write(`今日の夢: ${dreamSummaries}`, this.emotionEngine.getState(), 'observation', '夢');
        }
        this.events.emitSync({ type: 'woke_up', timestamp: tick, data: {} });
        if (this.ui) {
    ...
    // 13. 自己内省
    // ============================================================
    attemptSelfReflection(tick, timeOfDay) {
        // 内省の条件：夜間、感情が強い時、間隔を空ける
        const emotionalState = this.emotionEngine.getState();
        const shouldReflect = (timeOfDay === 'evening' || timeOfDay === 'night') ||
        ...
        if (reflection) {
            ...
            this.episodicMemory.formMemory({
                ...
                emotionalTags: [reflection.emotionalResponse.emotion || emotionalState.primary],
                emotionalIntensity: reflection.depth,
                relatedConcepts: ['内省', reflection.topic],
            });
            // 学習エンジンにも反映
            this.learnEngine.learnFromReflection(`${reflection.topic}についての内省: ${reflection.thoughtProcess[0]?.substring(0, 100) || ''}`, ['内省', reflection.topic]);
            if (this.ui) {
    ...
    // 14. 日記チェック
    // ============================================================
    checkDiaryWrite(tick, timeOfDay, day) {
        if (timeOfDay !== 'evening' && timeOfDay !== 'night')
            return;
        if (this.diaryState.lastReflectionDay >= day)
            return;
        const worldTime = this.time.getState();
    ...
    writeDiaryFromAction(tick, emotionalState) {
        const recentThoughts = this.thoughts.getRecentThoughts(5);
        const thoughtContent = recentThoughts.map(t => t.content).join('。');
        this.diary.write(thoughtContent || '今日もいろいろなことを考えた', emotionalState, 'thought', '日常');
        this.events.emitSync({
            type: 'diary_written',
            timestamp: tick,
            data: {
    ...
    // 16. 自己修正サイクル
    // ============================================================
    runSelfModificationCycle(tick, timeOfDay) {
        // 睡眠中は実行しない
        if (this.sleepState.isSleeping)
            return;
        const emotionalState = this.emotionEngine.getState();
     ...
     */
    runAutonomousEvolutionCycle(tick, timeOfDay, day) {
        if (this.sleepState.isSleeping)
            return;
        // --- 4A: 行動フィードバックの記録 ---
     ...
     */
    recordBehaviorOutcomes(tick, timeOfDay) {
        const currentAction = this.behavior.getCurrentAction();
        const emotionalState = this.emotionEngine.getState();
        ...
        if (recentActions.length >= 2) {
            ...
            const satisfaction = Math.max(0, Math.min(1, (emotionalState.valence + 1) / 2 * 0.6 + // 感情価: -1〜1 → 0〜0.6
            ...
            this.behavior.recordActionOutcome(`action_${lastAction}`, lastAction, context, {
                success,
                satisfaction,
    ...
    /** 行動結果の追跡用 */
    lastRecordedEnergy = 0.5;
    lastDiscoveryNotification = 0;
    lastExpansionCheck = 0;
    /**
     * BehaviorContextの構築
     */
    buildBehaviorContext(timeOfDay) {
        const urgeState = this.urgeSystem.getState();
        const emotionalState = this.emotionEngine.getState();
        const body = this.body;
        const timeState = this.time.getState();
     ...
     */
    checkWorldExpansion(tick, day) {
        // 1日1回チェック
        if (day === this.lastExpansionCheck)
            return;
    ...
    // 日の切り替わり
    // ============================================================
    checkDayTransition(day, timeOfDay) {
    ...
    // 訪問者対応
    // ============================================================
    async handleVisitorMessage(message) {
        ...
        try {
            ...
            const turnAnalysis = this.conversationEngine.analyzeVisitorTurn(message, tick, emotionalState.primary, recentThoughts.map(t => t.content));
            // --- Phase 5B: 関係性への反映 ---
            const relationship = this.relationshipEvolution.getRelationship();
            const flowState = this.conversationEngine.getFlowState();
            const depthLabel = flowState.depthScore >= 0.85 ? 'profound' :
            ...
            // --- Phase 5E: メッセージの記憶処理 ---
            const memoryResult = this.conversationMemoryBridge.processMessage('visitor', message, turnAnalysis.emotions[0] || null, turnAnalysis.topics, depthLabel, tick);
            // 記憶への書き込み
            ...
            this.visitorMemory.processMessage(message, turnAnalysis.emotions, turnAnalysis.topics, depthLabel, tick);
            // --- Phase 7: 連想ネットワーク活性化 ---
            for (const topic of turnAnalysis.topics) {
                this.associativeNetwork.activateByLabel(topic, 0.8, tick);
            }
            for (const emotion of turnAnalysis.emotions) {
                this.associativeNetwork.activateByLabel(emotion, 0.5, tick);
            }
            ...
            if (typeof this.conversationEngine.getRecentExchanges === 'function') {
                const exchanges = this.conversationEngine.getRecentExchanges(6);
                if (Array.isArray(exchanges))
                    recentExchanges.push(...exchanges);
            }
            const richContext = this.contextBridge.buildResponseContext(stateProvider, message, {
                intent: turnAnalysis.intent,
                emotions: turnAnalysis.emotions,
                topics: turnAnalysis.topics,
                hasSelfDisclosure: turnAnalysis.hasSelfDisclosure,
            }, {
                turnCount: flowState.turnCount || 0,
                depth: depthLabel,
                recentExchanges,
            }, phase, tick);
            // --- Phase 7: 自律戦略の決定（GradualAutonomy） ---
            const patternSituation = {
            ...
            let expressed;
            let structuredMeta = { detectedEmotion: null, detectedTopics: [], confidence: 0 };
            if (!this.autonomy.needsLLM(strategy)) {
                // パターンのみで応答（autonomous レベル）
                expressed = strategy.response;
            }
            else {
                ...
                switch (strategy.type) {
                    case 'llm_with_pattern_hints':
                        userPrompt += `\n\n参考（過去に好評だった応答）: 「${strategy.patternTemplate}」`;
                        break;
                    case 'pattern_draft_llm_refine':
                        userPrompt += `\n\n以下を元に自然に仕上げて: 「${strategy.draft}」`;
                        break;
                ...
                const structuredResult = await this.llm.expressResponseStructured(systemPrompt, userPrompt, '');
                ...
                if (!expressed || expressed.trim().length === 0) {
                    ...
                    expressed = await this.llm.expressResponse(turnAnalysis.topics.length > 0
            ...
            // --- Phase 7: 応答からのパターン抽出 ---
            const responseSatisfaction = Math.min(filterResult.qualityScore, validation.isValid ? 0.7 : 0.3);
            ...
            this.autonomy.reportQuality(responseSatisfaction, strategy.type !== 'llm_only', strategy.patternId, validation.isValid && !filterResult.wasFiltered);
            // === 応答を表示 ===
            if (this.ui) {
            ...
            // === Phase 5E: 応答の記憶処理 ===
            this.conversationMemoryBridge.processMessage('somunia', expressed, responseEmotionalTone, turnAnalysis.topics, depthLabel, tick);
            // 共有記憶の生成チェック
            if (this.relationshipEvolution.shouldCreateSharedMemory(message, depthLabel, turnAnalysis.emotions, turnAnalysis.hasSelfDisclosure)) {
                this.relationshipEvolution.createSharedMemory(message.substring(0, 60), `会話: ${turnAnalysis.topics.join(', ')}`, `訪問者と${depthLabel}な話をした`, turnAnalysis.emotions, depthLabel, tick);
            }
            // === 記録（既存Visitor互換） ===
            const currentThought = this.thoughts.getCurrentThought();
            this.visitor.recordResponse(expressed, responseEmotionalTone, currentThought?.content || '', tick);
            // === 感情の反映 ===
            this.emotionEngine.changeEmotions([
            ...
            if (depthLabel === 'intimate' || depthLabel === 'profound') {
                this.internalNarrative.recordSignificantEvent(`深い会話をした: ${turnAnalysis.topics.join(', ')}`, this.time.getDay());
            }
            // === 学習（会話から） ===
            if (turnAnalysis.topics.length > 0) {
                for (const topic of turnAnalysis.topics) {
                    this.learnEngine.learnFromExperience(`会話で${topic}について話した`, 'positive', [topic, '会話']);
                }
            }
            ...
            for (const topic of turnAnalysis.topics) {
                ...
                for (const other of turnAnalysis.topics) {
                    if (topic !== other) {
                        ...
                        this.associativeNetwork.connect(topic, other, 'related_to', 0.6);
                    }
                }
                // 感情との関連
                for (const emotion of turnAnalysis.emotions) {
                    this.associativeNetwork.connect(topic, emotion, 'feels_like', 0.4);
                }
            }
        }
        finally {
    ...
    buildStateProvider() {
        return {
            ...
            getRecentThoughts: (count) => this.thoughts.getRecentThoughts(count),
            getRecentEpisodicMemories: (count) => this.episodicMemory.getRecentMemories(count),
            getRecentSemanticLearnings: (count) => {
                const concepts = [];
                for (let i = 0; i < count; i++) {
                    const c = this.semanticMemory.getRandomConcept();
                    if (c)
                        concepts.push({ concept: c.concept });
                }
                return concepts;
            },
            getCreativeWorks: () => {
                // CreativeEngineに完了作品リストがない場合は空配列
                const works = this.creativeEngine.completedWorks;
                if (Array.isArray(works)) {
                    return works.slice(-5).map((w) => ({
                        type: w.type || 'unknown',
                        title: w.title || '無題',
            ...
            getCreativeWorksInProgress: () => this.creativeEngine.getWorksInProgress().map((w) => ({
                type: w.type || 'unknown',
                title: w.title || '制作中',
            })),
            getCurrentChapter: () => {
            ...
            getRecentGrowth: () => {
                const growth = this.selfModification.growthRecords;
            ...
            getRecentDreamSummary: () => {
                const sessions = this.dreamPhase.pastSessions;
            ...
            getBodyState: () => ({
                ...
                sleepiness: (1 - this.homeostasis.getState().energy.current), // sleepinessはenergyの逆
            }),
        };
    }
    /** 時間帯の表現を取得 */
    ...
    getThingToTell() {
        const concept = this.semanticMemory.getRandomConcept();
        if (concept)
            return `${concept.concept}のこと`;
        return null;
    }
    /** 過去の話題を取得 */
    getPastTopic(currentTopics) {
     ...
     */
    checkCreativeUrges(tick, timeOfDay) {
        if (this.sleepState.isSleeping)
            return;
        if (this.visitor.isPresent())
            return; // 訪問者がいる時は創作しない
        const emotionalState = this.emotionEngine.getState();
        const currentAction = this.behavior.getCurrentAction();
        // 既に創作中なら新しい衝動はチェックしない
        if (this.creativeEngine.getWorksInProgress().length > 0)
            return;
        const urge = this.creativeEngine.checkCreativeUrge(emotionalState.primary, emotionalState.levels[emotionalState.primary], this.thoughts.getRecentThoughts(3).map(t => t.content), this.episodicMemory.getRecentMemories(3).map(m => m.summary), [], // recentDreams
        ...
        if (urge) {
            ...
            // ナラティブに記録
            this.internalNarrative.recordSignificantEvent(`${work.type}の創作を始めた: 「${work.title}」`, this.time.getDay());
        }
    }
    /**
     * Phase 5C: 創作の進行
     */
    progressCreativeWorks(tick) {
        const worksInProgress = this.creativeEngine.getWorksInProgress();
        if (worksInProgress.length === 0)
            return;
        const emotionalState = this.emotionEngine.getState();
        for (const work of worksInProgress) {
            const result = this.creativeEngine.progressCreation(work.id, emotionalState.primary, this.thoughts.getRecentThoughts(1).map(t => t.content), tick);
            if (result && result.isComplete) {
                ...
                // ナラティブに記録
                this.internalNarrative.recordSignificantEvent(`${work.type}「${work.title}」を完成させた`, this.time.getDay());
            }
        }
    }
    /**
     * Phase 5D: 内的ナラティブの更新
     */
    updateInternalNarrative(tick, day) {
        if (this.sleepState.isSleeping)
            return;
        const emotionalState = this.emotionEngine.getState();
        const semanticStats = this.semanticMemory.getStats();
        ...
        // ナラティブのtick処理
        const narrativeUpdate = this.internalNarrative.tick(tick, day, emotionalState.primary, this.episodicMemory.getRecentMemories(5).map(m => m.summary), this.thoughts.getRecentThoughts(3).map(t => t.content), semanticStats.totalConcepts, conversationPatterns.totalConversations, creativeStats.totalWorks);
        // ナラティブから思考が生まれたら
    ...
    handleVisitorDeparture(tick) {
        ...
        // --- Phase 5E: 会話後の振り返り ---
        const reflection = this.conversationMemoryBridge.generatePostConversationReflection(`conv_${tick}`, conversationSummary, emotionalState.primary, relationship, tick);
        // 振り返りの内容を日記に記録
        this.diary.write(reflection.content, emotionalState, 'thought', '会話の振り返り');
        // 別れの感情
        this.emotionEngine.changeEmotions([
        ...
        if (conversationSummary.maxDepth > 0.5) {
            this.internalNarrative.recordSignificantEvent(`訪問者と深い会話をした（深度: ${(conversationSummary.maxDepth * 100).toFixed(0)}%）`, this.time.getDay());
        }
        // 既存のvisitorの退出
    ...
    handleCommand(command, args) {
        switch (command) {
            ...
            default:
                if (this.ui)
                    this.ui.showSystem(`不明なコマンド: ${command}`);
        }
    }
    // ============================================================
    // ヘルパーメソッド
    ...
    /** パターンの使用記録 */
    recordPatternUse(type, action) {
        const emotionalState = this.emotionEngine.getState();
        const matching = type === 'behavior'
            ? this.patternLibrary.findMatchingBehaviorPatterns(this.getUrgeMap(), this.time.getTimeOfDay(), emotionalState, this.body.energy)
            : this.patternLibrary.findMatchingSpeechPatterns(emotionalState, this.time.getTimeOfDay(), action, this.visitor.isPresent(), this.getUrgeMap());
        if (matching.length > 0) {
    ...
    // 表示メソッド
    // ============================================================
    displayStatus(timeOfDay, timeState) {
        if (!this.ui)
            return;
        const emotionalState = this.emotionEngine.getState();
        const currentAction = this.behavior.getCurrentAction();
  ...
  会話統計:
    ...
    registerPersistenceModules() {
        ...
        this.persistence.registerModule('homeostasis', {
            ...
            fromJSON: (data) => {
                if (data) {
                    const restored = Homeostasis_1.Homeostasis.fromJSON(data);
        ...
        this.persistence.registerModule('urgeSystem', {
            ...
            fromJSON: (data) => {
                if (data) {
                    const restored = UrgeSystem_1.UrgeSystem.fromJSON(data);
        ...
        this.persistence.registerModule('world', {
            ...
            fromJSON: (data) => { if (data)
                Object.assign(this.world, PixelWorld_1.PixelWorld.fromJSON(data)); },
        });
        this.persistence.registerModule('perception', {
            toJSON: () => this.perception.toJSON(),
            fromJSON: (data) => { if (data)
                Object.assign(this.perception, Perception_1.Perception.fromJSON(data)); },
        });
        ...
        this.persistence.registerModule('episodicMemory', {
            ...
            fromJSON: (data) => { if (data)
                Object.assign(this.episodicMemory, EpisodicMemory_1.EpisodicMemorySystem.fromJSON(data)); },
        });
        this.persistence.registerModule('semanticMemory', {
            toJSON: () => this.semanticMemory.toJSON(),
            fromJSON: (data) => { if (data)
                Object.assign(this.semanticMemory, SemanticMemory_1.SemanticMemorySystem.fromJSON(data)); },
        });
        this.persistence.registerModule('proceduralMemory', {
            toJSON: () => this.proceduralMemory.toJSON(),
            fromJSON: (data) => { if (data)
                Object.assign(this.proceduralMemory, ProceduralMemory_1.ProceduralMemorySystem.fromJSON(data)); },
        });
        this.persistence.registerModule('habitEngine', {
            toJSON: () => this.habitEngine.toJSON(),
            fromJSON: (data) => { if (data)
                Object.assign(this.habitEngine, HabitEngine_1.HabitEngine.fromJSON(data)); },
        });
        this.persistence.registerModule('diary', {
            toJSON: () => this.diary.toJSON(),
            fromJSON: (data) => { if (data)
                Object.assign(this.diary, Diary_1.Diary.fromJSON(data)); },
        });
        ...
        this.persistence.registerModule('dreamPhase', {
            toJSON: () => ({
                pastSessions: this.dreamPhase.pastSessions || [],
                dreamMaterials: this.dreamPhase.dreamMaterials || [],
            }),
            fromJSON: (data) => {
                if (data?.pastSessions) {
                    this.dreamPhase.pastSessions = data.pastSessions;
                }
            },
        });
        this.persistence.registerModule('learnEngine', {
            toJSON: () => ({
                sessions: this.learnEngine.sessions || [],
                tendency: this.learnEngine.tendency || {},
                goals: this.learnEngine.goals || [],
            }),
            fromJSON: (_data) => { },
        });
        this.persistence.registerModule('selfModification', {
            toJSON: () => ({
                reflections: this.selfModification.reflections || [],
                growthRecords: this.selfModification.growthRecords || [],
                selfConcepts: this.selfModification.selfConcepts || [],
                values: this.selfModification.values || [],
            }),
            fromJSON: (data) => {
                if (data?.reflections) {
                    this.selfModification.reflections = data.reflections;
                }
                if (data?.growthRecords) {
                    this.selfModification.growthRecords = data.growthRecords;
                }
            },
        });
        // --- メタ状態 ---
     ...
     * Wikipedia API接続テスト
     */
    async handleWikiConnectionTest() {
        if (!this.ui)
            return;
     ...
     * 永続化ステータス表示
     */
    showPersistenceStats() {
        if (!this.ui)
            return;
        const info = this.persistence.getStorageInfo();
        this.ui.showPersistenceStatus(info);
    }
    /**
     * テスト実行
     */
    ...
    // 外部アクセス（テスト・デバッグ用）
    // ============================================================
    getTickCount() { return this.tickCounter; }
    getEmotionEngine() { return this.emotionEngine; }
    getUrgeSystem() { return this.urgeSystem; }
    getHomeostasis() { return this.homeostasis; }
    getEpisodicMemory() { return this.episodicMemory; }
    getSemanticMemory() { return this.semanticMemory; }
    getProceduralMemory() { return this.proceduralMemory; }
    getPatternLibrary() { return this.patternLibrary; }
    getYuragi() { return this.yuragi; }
    getWorld() { return this.world; }
    getPerception() { return this.perception; }
    getHabitEngine() { return this.habitEngine; }
    getDiary() { return this.diary; }
    getDreamPhase() { return this.dreamPhase; }
    getLearnEngine() { return this.learnEngine; }
    getWikipediaLearner() { return this.wikipediaLearner; }
    getSelfModification() { return this.selfModification; }
    getBehaviorEngine() { return this.behavior; }
    getThoughtEngine() { return this.thoughts; }
    getVisitor() { return this.visitor; }
    getBody() { return this.body; }
    isSleeping() { return this.sleepState.isSleeping; }
    isLearning() { return this.learningState.isLearning; }
    // Phase 5 getters
    getConversationEngine() { return this.conversationEngine; }
    getRelationshipEvolution() { return this.relationshipEvolution; }
    getConversationMemoryBridge() { return this.conversationMemoryBridge; }
    getCreativeEngine() { return this.creativeEngine; }
    getInternalNarrative() { return this.internalNarrative; }
}
exports.SoulEngine = SoulEngine;
//# sourceMappingURL=SoulEngine.js.map