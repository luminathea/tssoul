...
exports.Yuragi = exports.yuragiSystem = exports.YuragiSystem = void 0;
...
class YuragiSystem {
     ...
     * ティック処理
     */
    tick(currentTick, emotionalState, urges, timeOfDay, fatigue, plannedAction) {
        this.currentTick = currentTick;
        // アクティブな揺らぎの終了をチェック
        this.updateActiveYuragi();
        // 揺らぎの発生判定
        const triggeredYuragi = this.checkYuragiTrigger(emotionalState, urges, timeOfDay, fatigue);
        let result = {
            triggered: null,
            thoughtIntrusion: null,
            actionOverride: null,
            emotionChanges: {},
            irrationalAction: null,
        };
        if (triggeredYuragi) {
            ...
            if (condition) {
                ...
                if (condition.effects.thoughtIntrusion && condition.effects.thoughtIntrusion.length > 0) {
                    let thought = condition.effects.thoughtIntrusion[Math.floor(Math.random() * condition.effects.thoughtIntrusion.length)];
                    // ランダム概念の置換
                    if (thought.includes('{{random_concept}}')) {
                        const concept = this.randomConcepts[Math.floor(Math.random() * this.randomConcepts.length)];
                ...
                // 揺らぎをアクティブにする
                const duration = Math.floor(this.config.maxDuration * condition.effects.durationMultiplier);
     ...
     * 揺らぎの発生をチェック
     */
    checkYuragiTrigger(emotionalState, urges, timeOfDay, fatigue) {
        // クールダウン中は発生しない
        ...
        for (const condition of YURAGI_CONDITIONS) {
            ...
            if (favorable.urgeThresholds) {
                for (const threshold of favorable.urgeThresholds) {
                    conditionCount++;
                    const urgeLevel = urges.urges[threshold.urge]?.level || 0;
    ...
    updateState() {
        // 気まぐれ度を非合理的行動の頻度から計算
        const recentActions = this.state.recentIrrationalActions.filter(a => Date.now() - a.timestamp < 3600000 // 1時間以内
        );
     ...
     */
    injectYuragi(type, intensity = 0.5) {
        const condition = YURAGI_CONDITIONS.find(c => c.type === type);
        if (!condition) {
            return { thoughtIntrusion: '', emotionChanges: {} };
        }
        // 揺らぎをアクティブにする
        const duration = Math.floor(this.config.maxDuration * condition.effects.durationMultiplier * intensity);
        this.activateYuragi(type, duration);
        // 思考を選択
        let thought = condition.effects.thoughtIntrusion?.[Math.floor(Math.random() * (condition.effects.thoughtIntrusion?.length || 1))] || '';
        if (thought.includes('{{random_concept}}')) {
            const concept = this.randomConcepts[Math.floor(Math.random() * this.randomConcepts.length)];
            thought = thought.replace(/\{\{random_concept\}\}/g, concept);
        }
        ...
        if (condition.effects.emotionChange) {
            for (const [emotion, change] of Object.entries(condition.effects.emotionChange)) {
                emotionChanges[emotion] = change * intensity;
            }
        }
    ...
    generateMicroYuragi() {
        // 常に微小な感情ノイズを生成
        const emotions = ['peace', 'melancholy', 'curiosity', 'loneliness', 'warmth'];
        const emotionNoise = {};
     ...
     * 確率的に異なる行動を選択させる
     */
    applyActionYuragi(sortedActions, topN = 3) {
     ...
     * 感情変化に揺らぎを適用
     */
    applyEmotionYuragi(baseChange) {
        const result = { ...baseChange };
        // 揺らぎによるノイズを追加
        ...
        if (Math.random() < this.state.unpredictability * 0.1) {
            const unexpectedEmotions = ['nostalgia', 'wonder', 'anxiety', 'hope'];
    ...
    getStats() {
        ...
        return {
            ...
            yuragiDistribution: distribution,
            averageWhimsicality: this.state.whimsicality,
            averageUnpredictability: this.state.unpredictability,
        };
    }
}
exports.YuragiSystem = YuragiSystem;
// デフォルトエクスポート
exports.yuragiSystem = new YuragiSystem();
/**
 * 簡易揺らぎ生成器（モジュール内部用）
 *
...
exports.Yuragi = Yuragi;
//# sourceMappingURL=Yuragi.js.map