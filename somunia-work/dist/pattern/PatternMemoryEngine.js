...
exports.PatternMemoryEngine = void 0;
// ============================================================
...
class PatternMemoryEngine {
     ...
     */
    findBestMatch(situation, variables, tick) {
        const matches = [];
        for (const pattern of this.patterns.values()) {
            // 最近使ったパターンは除外（繰り返し防止）
            if (this.recentlyUsed.includes(pattern.id))
                continue;
     ...
     * 各次元のマッチ度を重み付きで合計
     */
    calculateMatchScore(patternSit, currentSit) {
        let score = 0;
        let maxScore = 0;
        // インテント（最重要）
        ...
        if (patternSit.emotions.length === 0) {
            ...
            // 近い感情であれば部分的にマッチ
            const emotionSimilarity = this.calculateEmotionSimilarity(patternSit.emotions, currentSit.emotions);
        ...
        if (patternSit.keywords.length === 0) {
            ...
            const keywordMatch = patternSit.keywords.filter(k => currentSit.keywords.some(ck => ck.includes(k) || k.includes(ck))).length;
            if (keywordMatch > 0) {
     ...
     * 感情の類似度を計算
     */
    calculateEmotionSimilarity(a, b) {
        // 感情の近さマップ（簡易版）
        const emotionGroups = {
    ...
    expandTemplate(template, vars) {
        ...
        for (const [placeholder, value] of replacements) {
            if (value === null) {
                // オプショナルでない変数が欠けている場合
                if (['visitorName', 'timeExpression', 'moodExpression'].includes(placeholder.replace(/[{}]/g, ''))) {
    ...
    // 重複チェックと既存パターンの強化
    // ============================================================
    isDuplicate(template, situation) {
        for (const existing of this.patterns.values()) {
            ...
            if (similarity > this.config.duplicateThreshold) {
                ...
                if (situationOverlap > 0.5)
                    return true;
            }
        }
        return false;
    }
    reinforceExistingPattern(template, situation, satisfaction) {
        let bestMatch = null;
    ...
    templateSimilarity(a, b) {
        // 変数を統一してから比較
        const normalizeTemplate = (t) => t.replace(/\{[^}]+\}/g, '{}').replace(/\s+/g, '');
     ...
     * 応答検証の結果に基づいて呼ばれる
     */
    feedback(patternId, success, satisfaction) {
        const pattern = this.patterns.get(patternId);
        if (!pattern)
            return;
        if (success) {
    ...
    enforcePatternLimit() {
        ...
        for (let i = 0; i < toRemove && i < scored.length; i++) {
            // 初期パターンは保護
            if (scored[i].pattern.origin === 'initial')
                continue;
            this.patterns.delete(scored[i].pattern.id);
        }
    }
    calculatePatternValue(pattern) {
    ...
    cullLowQuality() {
        ...
        for (const pattern of this.patterns.values()) {
            // 初期パターンは保護
            if (pattern.origin === 'initial')
                continue;
            // 十分使われていないものはまだ判断しない
            if (pattern.useCount < this.config.minUsesBeforeCulling)
                continue;
            // 満足度が閾値以下 → 淘汰候補
     ...
     */
    getCoverage() {
        const intentCoverage = new Set();
        const emotionCoverage = new Set();
        const depthCoverage = new Set();
        for (const p of this.patterns.values()) {
            if (p.avgSatisfaction < 0.5)
                continue; // 低品質パターンは除外
        ...
        return (intentRate * 0.4 + emotionRate * 0.3 + depthRate * 0.3);
    }
    // ============================================================
    // 永続化
    // ============================================================
...
exports.PatternMemoryEngine = PatternMemoryEngine;
//# sourceMappingURL=PatternMemoryEngine.js.map