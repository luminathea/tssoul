...
exports.JsonToSqliteMigrator = void 0;
...
class JsonToSqliteMigrator {
    ...
    // 連想ネットワーク
    // ============================================================
    migrateAssociativeNetwork(data) {
        if (!data)
            return;
        try {
            // ノード
        ...
        try {
            ...
            for (const mem of memories) {
                this.db.saveSemanticMemory({
                    ...
                    relations: (mem.relatedConcepts || []).map((r) => ({
                        concept: r.concept,
        ...
        try {
            ...
            for (const profile of profiles) {
                ...
                for (const topic of topics) {
                    this.db.run('INSERT INTO visitor_topics (visitor_id, topic, discussed_at, depth) VALUES (?, ?, ?, ?)', profile.id, topic.topic, topic.discussedAt || 0, topic.depth || 'surface');
                }
                // 感情的結びつき
                const emotions = profile.emotionalAssociations || [];
                for (const emo of emotions) {
                    this.db.run('INSERT INTO visitor_emotions (visitor_id, emotion, reason, strength) VALUES (?, ?, ?, ?)', profile.id, emo.emotion, emo.reason || '', emo.strength ?? 0.5);
                }
                // 特別な思い出
                const specialMems = profile.specialMemories || [];
                for (const mem of specialMems) {
                    this.db.run('INSERT INTO visitor_special_memories (visitor_id, content, emotion, importance, timestamp) VALUES (?, ?, ?, ?, ?)', profile.id, mem.content, mem.emotion || 'peace', mem.importance ?? 0.5, mem.timestamp || 0);
                }
                // 話したいこと
                const thingsToTell = profile.thingsToTell || [];
                for (const thing of thingsToTell) {
                    this.db.run('INSERT INTO visitor_things_to_tell (visitor_id, content, added_at, priority, told) VALUES (?, ?, ?, ?, ?)', profile.id, thing.content, thing.addedAt || 0, thing.priority ?? 0.5, thing.told ? 1 : 0);
                }
            }
        }
        catch (err) {
    ...
    // 会話エンジン
    // ============================================================
    migrateConversationEngine(data) {
        if (!data)
            return;
        try {
        ...
        try {
            ...
            for (const dream of dreams) {
                this.db.saveDream({
                    ...
                    fragments: (dream.fragments || []).map((f) => ({
                        content: f.content || '',
                        type: f.type || 'image',
    ...
    migrateBlobModules(data) {
        ...
        for (const [moduleName, moduleData] of Object.entries(data)) {
            if (normalizedModules.has(moduleName))
                continue;
...
exports.JsonToSqliteMigrator = JsonToSqliteMigrator;
//# sourceMappingURL=JsonToSqliteMigrator.js.map