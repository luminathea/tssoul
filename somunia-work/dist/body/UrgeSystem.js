...
class UrgeSystem {
     ...
     * 初期レベルを計算
     */
    calculateInitialLevel(urgeType, char) {
        // somuniaの重要度に基づく基本値
     ...
     * 毎tickの更新
     */
    update(currentTick, timeOfDay, emotionLevels) {
        const events = [];
     ...
     * 自然な欲求増加
     */
    applyNaturalGrowth(events, timestamp) {
        for (const [urgeType, urgeState] of Object.entries(this.state.urges)) {
            const char = URGE_CHARACTERISTICS[urgeType];
     ...
     * 時間帯の影響を適用
     */
    applyTimeInfluence(events, timestamp, timeOfDay) {
        for (const [urgeType, urgeState] of Object.entries(this.state.urges)) {
            const char = URGE_CHARACTERISTICS[urgeType];
     ...
     * 感情の影響を適用
     */
    applyEmotionInfluence(events, timestamp, emotionLevels) {
        for (const [urgeType, urgeState] of Object.entries(this.state.urges)) {
            const char = URGE_CHARACTERISTICS[urgeType];
            let totalInfluence = 0;
            let contributingEmotion = null;
            let maxContribution = 0;
            for (const [emotionType, influence] of Object.entries(char.emotionalInfluence)) {
     ...
     * 欲求間の抑制を適用
     */
    applySuppression(events, timestamp) {
        for (const [urgeType, urgeState] of Object.entries(this.state.urges)) {
            const char = URGE_CHARACTERISTICS[urgeType];
    ...
    detectConflicts() {
        ...
        for (const [urge1, urge2] of conflictPairs) {
            ...
            if (level1 > this.config.conflictThreshold && level2 > this.config.conflictThreshold) {
                ...
                // 既存の葛藤を探す
                const existingConflict = this.state.conflicts.find(c => (c.urge1 === urge1 && c.urge2 === urge2) ||
     ...
     * 支配的欲求を見つける
     */
    findDominantUrge(urges) {
        let maxLevel = 0;
        let dominant = null;
        for (const [urgeType, urgeState] of Object.entries(urges)) {
    ...
    normalizeUrges() {
        for (const urgeState of Object.values(this.state.urges)) {
            urgeState.level = Math.max(this.config.minimumLevel, Math.min(1, urgeState.level));
        }
    }
    // ============================================================
    // 欲求の満足
     ...
     * 行動による欲求の満足
     */
    satisfyByAction(action, intensity = 1.0) {
        const events = [];
        const timestamp = Date.now();
        for (const [urgeType, urgeState] of Object.entries(this.state.urges)) {
            const char = URGE_CHARACTERISTICS[urgeType];
     ...
     * 特定の欲求を直接満足
     */
    satisfyUrge(urgeType, amount, source) {
     ...
     * 特定の欲求を増加
     */
    increaseUrge(urgeType, amount, source) {
        const urgeState = this.state.urges[urgeType];
        if (!urgeState)
            return null;
        const timestamp = Date.now();
     ...
     * 記憶を欲求に関連付け
     */
    associateMemory(urgeType, memoryId) {
        const urgeState = this.state.urges[urgeType];
     ...
     * 葛藤を解決しようとする
     */
    attemptResolveConflict(conflict) {
    ...
    getActiveUrges() {
        return Object.entries(this.state.urges)
            ...
            .map(([type, state]) => {
            const char = URGE_CHARACTERISTICS[type];
            return {
                urge: type,
                level: state.level,
    ...
    getSummary() {
        ...
        const conflicting = this.state.conflicts.map(c => [c.urge1, c.urge2]);
        let description = '';
     ...
     */
    getStats() {
        const satisfactions = this.changeHistory.filter(e => e.trigger.type === 'action_satisfaction').length;
        // 最も頻繁に高い欲求
        let maxAvg = 0;
...
exports.UrgeSystem = UrgeSystem;
exports.default = UrgeSystem;
//# sourceMappingURL=UrgeSystem.js.map