...
class Diary {
    ...
    streakData = { current: 0, longest: 0, lastDate: '' };
    constructor(config = {}) {
     ...
     * 思考の断片を書き込む
     */
    write(content, emotions, type = 'thought', trigger) {
     ...
     * 存在論的な問いを書き込む
     */
    writeExistentialQuestion(emotions) {
        if (Math.random() > this.config.existentialQuestionRate) {
            return null;
        }
        const question = POETIC_TEMPLATES.existentialQuestions[Math.floor(Math.random() * POETIC_TEMPLATES.existentialQuestions.length)];
        return this.write(question, emotions, 'question');
    }
    /**
     * 発見を記録
     */
    recordDiscovery(discovery, emotions, source) {
    ...
    // 詩的表現の生成
    // ============================================================
    poeticize(content, emotions, type) {
        // 既に詩的な場合はそのまま
        ...
        // 感情に応じたテンプレートを選択
        const templates = POETIC_TEMPLATES.emotions[dominantEmotion];
        if (templates && Math.random() < 0.3) {
            return this.fillTemplate(templates[Math.floor(Math.random() * templates.length)], content);
        }
        // 句読点と余韻を追加
    ...
    generateDaySummary(emotions, discoveries) {
    ...
    generateReflectionPoem(emotions) {
    ...
    // ヘルパーメソッド
    // ============================================================
    extractDominantEmotions(emotions) {
        const threshold = 0.3;
    ...
    getStats() {
        ...
        // 最も多い感情を見つける
        let mostCommonEmotion = 'serenity';
        let maxCount = 0;
     ...
     */
    async poeticizeWithLLM(content, emotions, type) {
        if (!this.llm)
     ...
     */
    async generateDiaryProse(reflection) {
        if (!this.llm)
        ...
        try {
            const prose = await this.llm.expressThought(`この1日を日記として書く: ${rawData}`, {
     ...
     * LLMを使った詩の生成
     */
    async generatePoemWithLLM(theme, emotions, fragments) {
        if (!this.llm)
        ...
        const dayContext = fragments
            ...
            .join('。');
        try {
            const poem = await this.llm.expressThought(`テーマ「${theme}」で短い詩を書く。今日の体験: ${dayContext}`, {
                currentEmotion: dominantEmotion,
                emotionalIntensity: this.calculateIntensity(emotions),
                timeOfDay: 'night',
     ...
     * LLM付きの1日の振り返り
     */
    async reflectOnDayWithLLM(worldTime, emotions) {
        ...
        try {
            const [prose, poem] = await Promise.all([
                ...
                this.generatePoemWithLLM(baseReflection.emotionalJourney[0] || 'today', emotions, this.currentDayFragments),
            ]);
            return {
                ...baseReflection,
                summary: prose,
     ...
     */
    async writeWithLLM(content, emotions, type, context) {
        // まず基本的な断片を作成
     ...
     * 断片をLLMで内面の声として深く言語化
     */
    async enrichFragmentWithLLM(fragment, emotions, context) {
        if (!this.llm)
            return null;
        const dominantEmotion = this.getDominantEmotion(emotions);
        // 文脈情報を構築
        const contextInfo = [fragment.content];
        if (context?.recentActivity)
            contextInfo.push(`直前の行動: ${context.recentActivity}`);
        if (context?.interactedObject)
            contextInfo.push(`触れたもの: ${context.interactedObject}`);
        if (context?.timeOfDay)
            contextInfo.push(`時間帯: ${context.timeOfDay}`);
        if (context?.weather)
            contextInfo.push(`天気: ${context.weather}`);
        // 最近の断片から文脈を取る
        const recentFragments = this.currentDayFragments
            .slice(-3)
            .map(f => f.content);
        try {
            const result = await this.llm.expressThought(`内面の声として日記に書く: ${contextInfo.join('。')}`, {
                currentEmotion: dominantEmotion,
     ...
     */
    async writeDreamDiary(dream, emotions) {
        const dreamSummary = this.formatDreamForDiary(dream);
        // LLMで夢の語りを生成
        if (this.llm) {
            try {
                const dominantEmotion = this.getDominantEmotion(emotions);
                const dreamProse = await this.llm.expressThought(`目覚めた直後の夢の記録: ${dreamSummary}`, {
                    currentEmotion: dominantEmotion,
                ...
                const fragment = this.write(`【夢の記録】${dreamProse}`, emotions, 'memory', '夢から覚めた');
        ...
        // LLMなしの夢日記
        return this.write(`【夢の記録】${dreamSummary}`, emotions, 'memory', '夢から覚めた');
    }
    /**
     * 夢を日記用にフォーマット
     */
    formatDreamForDiary(dream) {
        const parts = [];
        if (dream.title)
            parts.push(`「${dream.title}」という夢を見た。`);
        // ナラティブの要約
     ...
     * 内面の対話（自分との対話をLLMで生成）
     */
    async writeInnerDialogue(topic, emotions, context) {
        const fragments = [];
        const dominantEmotion = this.getDominantEmotion(emotions);
        if (this.llm) {
            try {
                // 問いかけの自分
                const questionSide = await this.llm.expressThought(`自分自身への問いかけ: ${topic}`, {
                    currentEmotion: dominantEmotion,
                ...
                fragments.push(this.write(`【問い】${questionSide}`, emotions, 'question'));
                // 応答する自分
                const answerSide = await this.llm.expressThought(`自分の問いへの応答: 「${questionSide}」に対して`, {
                    currentEmotion: dominantEmotion,
                    emotionalIntensity: 0.6,
                    timeOfDay: 'night',
                ...
                fragments.push(this.write(`【応答】${answerSide}`, emotions, 'thought'));
                return fragments;
            }
            catch {
                // フォールバック
        ...
        // LLMなしの内面対話
        fragments.push(this.write(`${topic}…ということについて考えている。`, emotions, 'question'));
        fragments.push(this.write(this.generateFallbackInnerResponse(topic, dominantEmotion), emotions, 'thought'));
        return fragments;
    }
    generateFallbackInnerResponse(topic, emotion) {
        const responses = {
            melancholy: [
     ...
     */
    async generateWeeklyRetrospective(emotions) {
        ...
        if (this.llm) {
            try {
                ...
                narrative = await this.llm.expressThought(`1週間を振り返って日記に書く:\n${summaryData}`, {
     ...
     * 感情トレンドの分析
     */
    analyzeEmotionTrend(entries) {
        if (entries.length < 2)
            return '判定不能';
        const positiveEmotions = new Set(['joy', 'contentment', 'excitement', 'wonder', 'gratitude', 'serenity']);
        const negativeEmotions = new Set(['melancholy', 'loneliness', 'anxiety', 'frustration', 'boredom']);
        // 前半と後半を比較
        const mid = Math.floor(entries.length / 2);
        const firstHalf = entries.slice(mid); // 古い方（entries はdescソート）
        const secondHalf = entries.slice(0, mid); // 新しい方
        const countPositive = (e) => e.reduce((sum, entry) => sum + entry.dominantEmotions.filter(em => positiveEmotions.has(em)).length, 0);
        const countNegative = (e) => e.reduce((sum, entry) => sum + entry.dominantEmotions.filter(em => negativeEmotions.has(em)).length, 0);
        ...
        if (secondPos > firstPos && secondNeg <= firstNeg)
            return '上向き（前向きな感情が増えている）';
        if (secondNeg > firstNeg && secondPos <= firstPos)
            return '下向き（ネガティブな感情が増えている）';
        if (secondPos > firstPos && secondNeg > firstNeg)
            return '活発（感情の振れ幅が大きくなっている）';
        return '安定（大きな変化なし）';
    }
    /**
     * 成長の指標を特定
     */
    identifyGrowthIndicators(entries) {
        const indicators = [];
        const totalDiscoveries = entries.reduce((sum, e) => sum + e.newDiscoveries.length, 0);
        if (totalDiscoveries >= 5)
            indicators.push(`${totalDiscoveries}個の新しい発見`);
        const totalPoetic = entries.reduce((sum, e) => sum + e.poeticMoments.length, 0);
        if (totalPoetic >= 3)
            indicators.push('詩的な表現が豊かになっている');
        const questionCount = entries.reduce((sum, e) => sum + e.unansweredQuestions.length, 0);
        if (questionCount >= 3)
            indicators.push('問いが増えている（知的好奇心の高まり）');
        // 感情の多様性
        const allEmotions = new Set();
        for (const entry of entries) {
            entry.dominantEmotions.forEach(e => allEmotions.add(e));
        }
        if (allEmotions.size >= 5)
            indicators.push(`${allEmotions.size}種類の感情を経験（感情の幅が広がっている）`);
        return indicators;
    }
    /**
     * 週間振り返りのフォールバック
     */
    fallbackWeeklyNarrative(data) {
        const emotionJP = {
            joy: '喜び', melancholy: '物寂しさ', wonder: '驚き',
            loneliness: '孤独', contentment: '満足', curiosity: '好奇心',
            nostalgia: '懐かしさ', anxiety: '不安', serenity: '穏やかさ',
     ...
     */
    async revisitPastEntry(entry, currentEmotions) {
        ...
        if (this.llm) {
            try {
                const reflection = await this.llm.expressThought(`${daysAgo}日前の日記を読み返した感想: 「${pastSummary.slice(0, 100)}」`, {
                    currentEmotion: dominantEmotion,
                    emotionalIntensity: 0.5,
                    timeOfDay: 'afternoon',
                ...
                return this.write(`【読み返し：${entry.date}】${reflection}`, currentEmotions, 'memory', `${daysAgo}日前の日記を読み返した`);
            }
            catch {
                // フォールバック
            }
        ...
        return this.write(fallbacks[Math.floor(Math.random() * fallbacks.length)], currentEmotions, 'memory', `${daysAgo}日前の日記を読み返した`);
    }
    /**
     * 手紙を書く（未来の自分へ、または過去の自分へ）
     */
    async writeLetterToSelf(direction, emotions, context) {
        const dominantEmotion = this.getDominantEmotion(emotions);
        if (this.llm) {
            try {
                ...
                const prompt = context
                    ...
                    : `${target}への短い手紙を書く`;
                const letter = await this.llm.expressThought(prompt, {
                    currentEmotion: dominantEmotion,
                ...
                return this.write(`【${direction === 'future' ? '未来' : '過去'}の自分へ】${letter}`, emotions, 'poem');
            }
            catch {
                // フォールバック
            }
        ...
        return this.write(`【${direction === 'future' ? '未来' : '過去'}の自分へ】${letters[Math.floor(Math.random() * letters.length)]}`, emotions, 'poem');
    }
    /**
     * Wikipedia学習の感想を日記に書く
     */
    async writeLearnReflection(articleTitle, learnedContent, emotions) {
        const dominantEmotion = this.getDominantEmotion(emotions);
        if (this.llm) {
            try {
                const reflection = await this.llm.expressThought(`「${articleTitle}」について学んだ感想を日記に書く。学んだ内容: ${learnedContent.slice(0, 150)}`, {
                    currentEmotion: dominantEmotion,
                    emotionalIntensity: 0.5,
                    timeOfDay: 'afternoon',
                ...
                return this.write(`【学び：${articleTitle}】${reflection}`, emotions, 'discovery', `${articleTitle}について学んだ`);
            }
            catch {
                // フォールバック
            }
        }
        return this.write(`【学び：${articleTitle}】${learnedContent.slice(0, 80)}…新しいことを知った。`, emotions, 'discovery', `${articleTitle}について学んだ`);
    }
    /**
     * 訪問者との会話後の感想を日記に書く
     */
    async writeVisitorReflection(visitorInteraction, emotions) {
        const dominantEmotion = this.getDominantEmotion(emotions);
        if (this.llm) {
            try {
                const reflection = await this.llm.expressThought(`訪問者との会話を振り返って日記に書く: ${visitorInteraction.slice(0, 150)}`, {
                    currentEmotion: dominantEmotion,
                    emotionalIntensity: 0.6,
                    timeOfDay: 'evening',
                ...
                return this.write(`【会話の記録】${reflection}`, emotions, 'observation', '訪問者との会話');
            }
            catch {
                // フォールバック
            }
        }
        return this.write(`【会話の記録】誰かと話した。${visitorInteraction.slice(0, 50)}…`, emotions, 'observation', '訪問者との会話');
    }
    /**
     * フォールバック: コードのみで散文化
     */
    fallbackDiaryProse(reflection) {
        const parts = [];
