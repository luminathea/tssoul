...
exports.VisitorMemorySystem = void 0;
// ============================================================
// 名前検出パターン
// ============================================================
const NAME_PATTERNS = [
...
class VisitorMemorySystem {
    ...
    defaultVisitorId = 'visitor_default';
    constructor(config) {
    ...
    // メッセージ処理
    // ============================================================
    processMessage(message, emotions, topics, depth, tick) {
        if (!this.currentVisitorId) {
        ...
        if (emotions.length > 0 && depth !== 'surface') {
            ...
            if (ea) {
                ea.strength = Math.min(1, ea.strength + 0.05);
            }
    ...
    // 情報抽出
    // ============================================================
    extractFacts(message, profile) {
        const facts = [];
        for (const rule of EXTRACTION_RULES) {
            const match = message.match(rule.regex);
            if (match) {
                const content = rule.format(match);
                const dup = profile.knownFacts.some(f => f.content === content || (f.category === rule.category && this.similar(f.content, content)));
                if (!dup) {
                    const fact = {
    ...
    // コミュニケーションスタイル分析
    // ============================================================
    updateComStyle(profile, message) {
        ...
        if (phrases) {
            for (const p of phrases) {
                ...
                if (cleaned && !s.characteristicPhrases.includes(cleaned)) {
                    ...
                    if (s.characteristicPhrases.length > 8)
                        s.characteristicPhrases.shift();
                }
            }
        }
    }
    ...
    // コンテキスト生成（LLMに渡す）
    // ============================================================
    generateVisitorContext(currentTopics, currentMessage, relationshipPhase) {
        if (!this.currentVisitorId)
            return null;
        const profile = this.profiles.get(this.currentVisitorId);
        if (!profile)
    ...
    // 「話したいこと」の管理
    // ============================================================
    addThingToTell(content, priority = 0.5) {
    ...
    addThingToProfile(profile, content, priority) {
    ...
    selectRelevantFacts(profile, topics, message) {
    ...
    getRecentTopics(profile, count) {
        const seen = new Set();
        return profile.topicHistory.slice(-count * 2).reverse()
            .filter(t => { if (seen.has(t.topic))
            return false; seen.add(t.topic); return true; })
            .slice(0, count)
            .map(t => t.topic);
    }
    describeTimeSince(timestamp) {
    ...
    fromJSON(data) {
        ...
        if (data.defaultVisitorId)
            this.defaultVisitorId = data.defaultVisitorId;
    }
}
exports.VisitorMemorySystem = VisitorMemorySystem;
//# sourceMappingURL=VisitorMemorySystem.js.map