...
class ExpressionFilter {
    ...
    filter(rawResponse, context) {
        ...
        // 品質スコアの算出
        const qualityScore = this.calculateQuality(response, rawResponse, filterReasons.length, context);
        // 履歴に記録
     ...
     * 内部メッセージの漏れ検出
     */
    detectInternalMessage(response) {
    ...
    generateFallback(context) {
        ...
        const matchedTemplates = FALLBACK_TEMPLATES.filter(t => t.intents.includes(context.intent) &&
    ...
    // 品質スコア算出
    // ============================================================
    calculateQuality(filtered, original, issueCount, context) {
        let score = 1.0;
        // フィルタリングが必要だった場合
    ...
    fromJSON(data) {
        ...
        if (data.selfIntroCount !== undefined)
            this.selfIntroCount = data.selfIntroCount;
    }
}
exports.ExpressionFilter = ExpressionFilter;
//# sourceMappingURL=ExpressionFilter.js.map