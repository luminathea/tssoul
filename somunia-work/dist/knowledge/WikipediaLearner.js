...
exports.WikipediaLearner = void 0;
const Yuragi_1 = require("../brain/Yuragi");
const WikipediaAPI_1 = require("./WikipediaAPI");
// ============================================================
// somuniaの興味・感性定義
// ============================================================
/** somuniaが惹かれるテーマ */
...
class WikipediaLearner {
    ...
    constructor(config = {}) {
        ...
        this.api = new WikipediaAPI_1.WikipediaAPI();
     ...
     * 記事を読む
     */
    async readArticle(title, style, currentFatigue, currentMood) {
        ...
        // 読書時間をシミュレート
        const readingTime = this.calculateReadingTime(article, sectionsToRead, style, currentFatigue, interestLevel);
        // 理解度を計算
        const comprehension = this.calculateComprehension(style, currentFatigue, interestLevel, currentMood);
        // 心に残る言葉を抽出
        ...
        // 発見をチェック
        const discoveries = this.checkForDiscoveries(article, comprehension, interestLevel, memorableWords);
        // 次に辿るリンクを決定
        const followedLink = this.decideNextLink(article, style, interestLevel, currentFatigue);
     ...
     * 読むセクションを決定
     */
    decideSectionsToRead(article, style, interest) {
        const sections = ['summary']; // 要約は必ず読む
     ...
     * セクションへの興味を計算
     */
    calculateSectionInterest(section) {
        let interest = 0.3;
     ...
     * 読書時間を計算
     */
    calculateReadingTime(article, sectionsToRead, style, fatigue, interest) {
        // 読む文字数を計算
     ...
     * 理解度を計算
     */
    calculateComprehension(style, fatigue, interest, mood) {
     ...
     * 心に残る言葉を抽出
     */
    extractMemorableWords(article, sectionsRead) {
        const memorable = [];
     ...
     * 感想・メモを生成
     */
    generateNotes(article, comprehension, interest) {
        const notes = [];
        // 理解度に応じた感想
     ...
     * 発見をチェック
     */
    checkForDiscoveries(article, comprehension, interest, memorableWords) {
        const discoveries = [];
        const now = Date.now();
        // 事実の発見
        ...
        if (memorableWords.length > 0 && interest > 0.5) {
            discoveries.push({
                ...
                relatedArticle: article.title,
                emotionalResponse: POETIC_IMPRESSIONS.beauty[Math.floor(Math.random() * POETIC_IMPRESSIONS.beauty.length)],
        ...
        if (this.state.currentSession && this.state.currentSession.visitedArticles.length > 1) {
            const prevArticle = this.state.currentSession.visitedArticles[this.state.currentSession.visitedArticles.length - 1];
            if (this.findConnection(prevArticle.title, article.title)) {
                discoveries.push({
                    type: 'connection',
                    content: `${prevArticle.title}と${article.title}の間に繋がりを見つけた`,
                    relatedArticle: article.title,
                    emotionalResponse: POETIC_IMPRESSIONS.connection[Math.floor(Math.random() * POETIC_IMPRESSIONS.connection.length)],
                    significance: 0.8,
                    discoveredAt: now,
                });
            }
        ...
        if (interest > 0.7 && Math.random() < interest * 0.2) {
            discoveries.push({
                ...
                relatedArticle: article.title,
                emotionalResponse: POETIC_IMPRESSIONS.resonance[Math.floor(Math.random() * POETIC_IMPRESSIONS.resonance.length)],
                significance: 0.9,
                discoveredAt: now,
            });
        }
        ...
        if (comprehension < 0.5 && interest > 0.4) {
            discoveries.push({
                ...
                relatedArticle: article.title,
                emotionalResponse: POETIC_IMPRESSIONS.curiosity[Math.floor(Math.random() * POETIC_IMPRESSIONS.curiosity.length)],
                significance: 0.6,
                discoveredAt: now,
            });
        }
    ...
    findConnection(title1, title2) {
        ...
        if (!article1 || !article2)
            return false;
        // カテゴリの共通性
        const commonCategories = article1.categories.filter(c => article2.categories.includes(c));
     ...
     * 次に辿るリンクを決定
     */
    decideNextLink(article, style, interest, fatigue) {
        // リンクを辿る確率
     ...
     * お気に入りトピックを更新
     */
    updateFavoriteTopics(article, interest, discoveryCount) {
        // 興味が高ければお気に入りに追加/更新
        if (interest < 0.5)
            return;
        // 記事のメイントピックを抽出
        ...
        if (this.state.favoriteTopics.length > this.config.maxFavoriteTopics) {
            ...
            this.state.favoriteTopics = this.state.favoriteTopics.slice(0, this.config.maxFavoriteTopics);
        }
    }
    /**
     * 記事からトピックを抽出
     ...
     * セッションの感想を生成
     */
    generateSessionImpression(session) {
        const articleCount = session.visitedArticles.length;
        const discoveryCount = session.discoveries.length;
        const avgInterest = session.visitedArticles.reduce((sum, a) => sum + a.interestLevel, 0) / articleCount || 0;
        const parts = [];
        // 探索の量について
        if (articleCount >= 5) {
     ...
     */
    async fetchArticleOnline(title) {
     ...
     * Phase 6A: オンライン検索
     */
    async searchOnline(query, limit = 5) {
        return this.api.search(query, limit);
    }
    /**
     * Phase 6A: ランダム記事を取得
     */
    async fetchRandomOnline() {
     ...
     */
    getStatistics() {
        const totalArticlesRead = this.state.pastSessions.reduce((sum, s) => sum + s.visitedArticles.length, 0);
    ...
    static deserialize(data) {
        ...
        if (data.apiStatus)
            learner.api.fromJSON(data.apiStatus);
...
exports.WikipediaLearner = WikipediaLearner;
//# sourceMappingURL=WikipediaLearner.js.map