...
exports.ConversationMemoryBridge = void 0;
const LEARNING_TRIGGERS = [
    ...
    {
        description: '共有の瞬間',
        match: (msg, _) => msg.includes('一緒に') || msg.includes('二人で') || msg.includes('ね！') ||
...
class ConversationMemoryBridge {
    ...
    reflections = [];
    // --- 現在の会話の追跡 ---
    currentConversationLearnings = [];
    currentMessages = [];
    currentEmotionalArc = [];
    currentTopics = [];
    somuniaSelfDisclosures = [];
    visitorDisclosures = [];
    // --- 会話パターン分析 ---
     ...
     */
    processMessage(speaker, content, emotion, topics, depth, tick) {
        // メッセージの記録
        this.currentMessages.push({ speaker, content, emotion });
        if (emotion)
            this.currentEmotionalArc.push(emotion);
        ...
        if (speaker === 'visitor') {
            ...
            if (expression) {
                ...
                if (this.learnedExpressions.length > 50)
                    this.learnedExpressions.shift();
     ...
     * 会話終了時の振り返り生成
     */
    generatePostConversationReflection(conversationId, summary, currentEmotion, relationship, tick) {
        // 会話品質の評価
        ...
        // 振り返りの内容を生成
        const reflectionContent = this.generateReflectionContent(summary, quality, memorable, currentEmotion);
        const reflection = {
            conversationId,
     ...
     * 過去の会話から関連する学びを検索
     */
    searchRelevantLearnings(topics, emotion) {
        return this.learnings
            .filter(learning => {
            // トピックマッチ
            const topicMatch = topics.some(t => learning.content.includes(t) || learning.conversationContext.includes(t));
            // 感情マッチ
     ...
     */
    generatePastReference(currentTopics) {
        for (const topic of currentTopics) {
            const relatedLearning = this.learnings.find(l => l.content.includes(topic) && l.type !== 'self_discovery');
            if (relatedLearning) {
     ...
     * 学習内容の抽出
     */
    extractLearningContent(message, type, speaker) {
        // メッセージから核心部分を抽出
     ...
     * 重要度の計算
     */
    calculateImportance(baseImportance, depth, content) {
     ...
     * エピソード記憶を生成すべきか判定
     */
    shouldCreateEpisodic(content, speaker, depth, emotion) {
        // 深い会話は常に記憶
        if (depth === 'intimate' || depth === 'profound')
            return true;
        // 強い感情を伴う場合
        const strongEmotions = ['joy', 'melancholy', 'wonder', 'warmth'];
        if (strongEmotions.includes(emotion))
            return Math.random() < 0.5;
        // 長い自己開示
        if (content.length > 50 && this.isSelfDisclosure(content))
            return true;
        // 一定間隔で記録（10メッセージに1回）
        if (this.currentMessages.length % 10 === 0)
            return true;
        return false;
    }
    /**
     * エピソード記憶のフォーマット
     */
    formatEpisodicContent(content, speaker, emotion) {
     ...
     * エピソード記憶のサマリー
     */
    generateEpisodicSummary(content, speaker) {
        if (speaker === 'visitor') {
     ...
     * 学習可能な表現の検出
     */
    detectLearnableExpression(message) {
        // 感嘆表現
     ...
     * 会話品質の評価
     */
    evaluateConversationQuality(summary) {
        return {
    ...
    calculateReciprocity() {
        if (this.somuniaSelfDisclosures.length === 0 && this.visitorDisclosures.length === 0)
            return 0.5;
    ...
    calculateEmotionalResonance() {
        if (this.currentEmotionalArc.length < 2)
            return 0.3;
        // 感情の多様性と一貫性のバランス
     ...
     * 次に話したいことの生成
     */
    generateFutureTopics(summary, discussedTopics) {
        const futureTopics = [];
        // 深掘りしたかったトピック
     ...
     * 自分のパフォーマンス評価
     */
    evaluateSelfPerformance(quality) {
     ...
     * 関係性への影響の計算
     */
    calculateRelationshipImpact(quality, summary) {
     ...
     * 振り返りの内容を生成
     */
    generateReflectionContent(summary, quality, memorable, currentEmotion) {
        const parts = [];
        // 全体的な印象
     ...
     * 全体的な印象の決定
     */
    determineOverallImpression(quality, currentEmotion) {
        if (quality.emotionalResonance > 0.7)
            return 'warmth';
        if (quality.depthAchievement > 0.7)
            return 'wonder';
        if (quality.discoveryCount > 3)
            return 'curiosity';
    ...
    // パターン分析
    // ============================================================
    updatePatternAnalysis(summary, quality) {
     ...
     * 長所の特定
     */
    identifyStrengths(recentReflections) {
        const strengths = [];
        ...
        if (avgSatisfaction > 0.6)
            strengths.push('会話を楽しめている');
        const totalLearnings = recentReflections.reduce((sum, r) => sum + r.learnings.length, 0);
        if (totalLearnings > 10)
            strengths.push('会話から多くを学んでいる');
        const avgDepth = this.conversationPatterns.averageDepth;
        if (avgDepth > 0.5)
            strengths.push('深い話ができる');
        return strengths;
    }
    /**
     * 弱点の特定
     */
    identifyWeaknesses(recentReflections) {
        const weaknesses = [];
        const avgSatisfaction = recentReflections.reduce((sum, r) => sum + r.selfSatisfaction, 0) /
            Math.max(1, recentReflections.length);
        if (avgSatisfaction < 0.4)
            weaknesses.push('もっと自然に話せるようになりたい');
        const avgDepth = this.conversationPatterns.averageDepth;
        if (avgDepth < 0.3)
            weaknesses.push('表面的な会話が多い');
        return weaknesses;
    }
    /**
     * 成長提案の生成
     */
    generateGrowthSuggestions(recentReflections) {
        const suggestions = [];
        // 未探索のトピック
    ...
    // ゲッター
    // ============================================================
    getLearnings() { return [...this.learnings]; }
    getReflections() { return [...this.reflections]; }
    getLatestReflection() {
        return this.reflections.length > 0 ? this.reflections[this.reflections.length - 1] : null;
    }
    getConversationPatterns() { return { ...this.conversationPatterns }; }
    getLearnedExpressions() { return [...this.learnedExpressions]; }
    getCurrentLearnings() { return [...this.currentConversationLearnings]; }
    /**
     * 特定タイプの学びを取得
     */
    ...
    fromJSON(data) {
        ...
        if (data.learnedExpressions)
            this.learnedExpressions = data.learnedExpressions;
    }
}
exports.ConversationMemoryBridge = ConversationMemoryBridge;
//# sourceMappingURL=ConversationMemoryBridge.js.map