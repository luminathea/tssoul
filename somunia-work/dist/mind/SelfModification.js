...
exports.SelfModification = void 0;
const Yuragi_1 = require("../brain/Yuragi");
// ============================================================
// somuniaの初期自己概念
// ============================================================
const INITIAL_SELF_CONCEPTS = [
    // アイデンティティ
    {
        aspect: 'identity',
...
class SelfModification {
    ...
    reflect(type, topic, context) {
        ...
        // 感情的反応
        const emotionalResponse = this.determineEmotionalResponse(reflectionTopic, insights, context?.currentEmotion);
        // 自己評価への影響
        const selfEvaluationImpact = this.assessSelfEvaluationImpact(reflectionTopic, insights, emotionalResponse);
        // 深さを計算
    ...
    selectReflectionTopic(context) {
        ...
        // ランダムにカテゴリを選択
        const categories = Object.keys(REFLECTION_TOPICS);
     ...
     * 思考プロセスを生成
     */
    generateThoughtProcess(topic, context) {
        const thoughts = [];
        // 開始の思考
        thoughts.push(`${topic}...`);
        // 関連する自己概念を参照
        const relatedConcepts = this.selfConcepts.filter(c => c.content.includes(topic.slice(0, 5)) || Math.random() < 0.2);
        if (relatedConcepts.length > 0) {
            const concept = relatedConcepts[Math.floor(Math.random() * relatedConcepts.length)];
            thoughts.push(`私は「${concept.content}」と思っていた`);
        }
        // 価値観を参照
        const relatedValues = this.values.filter(v => v.description.includes(topic.slice(0, 5)) || Math.random() < 0.2);
     ...
     * 洞察を導出
     */
    deriveInsights(topic, thoughtProcess) {
        const insights = [];
        // 思考プロセスの深さに応じて洞察を生成
    ...
    generatePartialUnderstanding(topic) {
        const partials = [
            '自分の傾向',
            '感情の流れ',
            '大切にしていること',
     ...
     * 感情的反応を決定
     */
    determineEmotionalResponse(topic, insights, currentEmotion) {
        // トピックと洞察に基づいて感情を決定
     ...
     * 自己評価への影響を判定
     */
    assessSelfEvaluationImpact(topic, insights, emotionalResponse) {
        // 洞察がない場合は影響なし
        if (insights.length === 0)
            return undefined;
        // アスペクトを決定
     ...
     * 内省の深さを計算
     */
    calculateReflectionDepth(thoughtProcess, insights) {
        let depth = 0;
        // 思考の量
     ...
     * 内省から自己概念を更新
     */
    updateSelfConceptsFromReflection(reflection) {
     ...
     * 新しい自己概念を追加
     */
    addSelfConcept(aspect, content, confidence, experience) {
        const now = Date.now();
        const concept = {
            aspect,
            content,
     ...
     * 自己概念を更新
     */
    updateSelfConcept(aspect, oldContent, newContent, reason) {
        const concept = this.selfConcepts.find(c => c.aspect === aspect && c.content === oldContent);
        if (!concept)
            return false;
        // 変化履歴を記録
     ...
     * アスペクト別の自己概念を取得
     */
    getSelfConceptsByAspect(aspect) {
     ...
     * 価値観を追加
     */
    addValue(name, description, importance, experience) {
        // 既存チェック
        ...
        // 成長記録
        this.recordGrowth('understanding', `新しい価値観の形成: ${name}`, '', description);
        return value;
    }
    /**
     * 価値観の葛藤をチェック
     */
    checkValueConflict(value1Name, value2Name) {
     ...
     * 目標を設定
     */
    setGoal(content, type, importance, relatedValues) {
        const goal = {
        ...
        if (this.goals.length > this.config.maxGoals) {
            const toRemove = this.goals.find(g => g.status === 'achieved' || g.status === 'abandoned');
            if (toRemove) {
    ...
    updateGoalProgress(goalId, progress) {
        ...
        if (goal.progress >= 1) {
            ...
            this.recordGrowth('skill', `目標達成: ${goal.content}`, '進行中', '達成');
        }
        return true;
    }
    /**
     ...
     * 性格特性に影響を与える
     */
    influencePersonality(traitName, direction, intensity) {
        ...
        if (Math.abs(trait.value - oldValue) > 0.05) {
            this.recordGrowth('emotional', `${traitName}の変化`, `${(oldValue * 100).toFixed(0)}%`, `${(trait.value * 100).toFixed(0)}%`);
        }
    }
    /**
     * 性格特性を取得
     */
    getPersonalityTrait(name) {
     ...
     * 成長を記録
     */
    recordGrowth(type, description, previousState, newState, trigger) {
        const record = {
    ...
    evaluateSelf(aspect) {
        ...
        if (recentReflections.length >= 3) {
            const positiveCount = recentReflections.filter(r => r.selfEvaluationImpact?.direction === 'positive').length;
            const negativeCount = recentReflections.filter(r => r.selfEvaluationImpact?.direction === 'negative').length;
    ...
    generateEvaluationDetails(aspect, score, trend) {
    ...
    generateModificationProposals(context) {
        ...
        if (this.externalModules.patternLibrary) {
            ...
            if (evolveProposal)
                newProposals.push(evolveProposal);
        }
        // 提案を保存
     ...
     * 感情調整の提案
     */
    proposeEmotionAdjustments(reflections, context) {
        const proposals = [];
        ...
        // 内省でネガティブな感情が多い場合
        const negativeCount = reflections.filter(r => r.emotionalResponse &&
            ['melancholy', 'loneliness', 'anxiety'].includes(r.emotionalResponse.emotion)).length;
     ...
     * 行動パターン変異の提案
     */
    proposeBehaviorMutations(context) {
        const proposals = [];
        // 行動成功率が低い場合
     ...
     * 性格パラメータの調整提案
     */
    proposePersonalityAdjustments(reflections, growth) {
        const proposals = [];
        // 成長が活発なら「好奇心」を上げる提案
        const recentGrowthCount = growth.filter(g => Date.now() - g.timestamp < 7 * 24 * 60 * 60 * 1000).length;
        ...
        const lonelyReflections = reflections.filter(r => r.topic.includes('孤独') || r.topic.includes('一人') || r.topic.includes('繋がり'));
     ...
     * 価値観の更新提案
     */
    proposeValueUpdates(reflections) {
        const proposals = [];
        // 内省から繰り返し現れるテーマを検出
     ...
     * 新しい目標の提案
     */
    proposeNewGoals(reflections, growth) {
        const proposals = [];
     ...
     */
    executeProposals(confidenceThreshold = 0.5) {
        const results = [];
        ...
        for (const proposal of pendingProposals) {
            ...
            // 成長記録
            if (result.success) {
                this.recordGrowth('self_modification', result.description, proposal.description, result.result || '実行済み', '自己修正');
            }
        }
        return results;
    }
     ...
     * 感情調整を実行
     */
    executeEmotionAdjustment(proposal) {
     ...
     * パターン変異を実行
     */
    executePatternMutation(proposal) {
        const { params } = proposal;
        if (!this.externalModules.patternLibrary) {
            return {
                proposalId: proposal.id,
        ...
        switch (params.action) {
            case 'trigger_evolution': {
                if (typeof library.evolvePatterns === 'function') {
                    ...
                    const totalChanges = (evolveResult.mutations?.length || 0) +
     ...
     * 性格調整を実行
     */
    executePersonalityAdjustment(proposal) {
        const { params } = proposal;
        this.influencePersonality(params.trait, params.direction, params.amount / this.config.personalityChangeRate // influencePersonalityが内部で乗算するため
        );
     ...
     * 目標追加を実行
     */
    executeGoalAddition(proposal) {
        const { params } = proposal;
        const goal = this.setGoal(params.content, params.type || 'growth', params.importance || 0.5, params.relatedValues || []);
     ...
     * パターン進化を実行
     */
    executePatternEvolution(proposal) {
        if (!this.externalModules.patternLibrary) {
            return {
                proposalId: proposal.id,
                success: false,
...
exports.SelfModification = SelfModification;
//# sourceMappingURL=SelfModification.js.map